---
title: "Predict pre-dawn leaf-level net assimilation with hyperspectral data."
date: "`r Sys.Date ()`"
output: pdf_document
---
...
2. Built a PLSR model with $W_{PLSR}$ as input and pre-dawn leaf-level assimilation (A, $\mu molm^{-2}s^{-1}$, *n* = 94) as output. Methods were adapted from Burnett et al. (2021).  

```{r, include=FALSE}
## Load datasets  
#ref: Weekly ID-based data
...
inVar <- "A"
file_path <- "./results/reliefF_plsr/assimilation/"
...
ref_trait_expRank <- attrEval(A ~., ref_trait_num, estimator = "RReliefFexpRank", ReliefIterations = nrow(trait)*50) 
...
A_feature_plot
```
Figure 1: Attributes values of HSI data on Week 6 for pre-dawn leaf-level assimilation (A, $\mu molm^{-2}s^{-1}$). Wavelengths with the top 300 attribute values are considered as important (red, $W_{PLSR}$) and are used for model input. 

```{r att anotate output, echo=FALSE, eval = F, fig.align='center', fig.width = 10, fig.height = 4}
A_feature_plot
ggsave(filename = paste0(file_path, "A_feature.png"), plot = A_feature_plot, device = "png", width = 30, height = 12, units = "cm", dpi = 400)
```
...
The data were first grouped by treatments. Then, 80% of the data were sampled for the calibration dataset.  
...
val_hist_plot <- qplot(val.plsr.data$A, geom="histogram", bins =7,
                       main = "Val. (n = 19)",
                       xlab = expression("A ("~mu*mol*m^-2*s^-1~")"), ylab = "Count",
                       fill=I("#0072B2"),col=I("black")) + theme_bw(base_size = 18)
...
```
Figure 3: Frequency distributions of pre-dawn leaf-level assimilation (A, $\mu molm^{-2}s^{-1}$) in the calibration (left, *n* = 75) and validation (right, *n* = 19) datasets.   
...
#png(filename = paste0(file_path, "A_nComps.png"), bg = "white", width = 7, height = 5, units = "in", res = 400)
...
cal.plsr.output <- data.frame(A = cal.plsr.data$A,
                              Treatment = cal.plsr.data$Treatment,
                              PLSR_Predicted=fit,
                              PLSR_CV_Predicted=as.vector(plsr.out$validation$pred[,,nComps]))
cal.plsr.output <- cal.plsr.output %>%
  mutate(PLSR_CV_Residuals = PLSR_CV_Predicted-A)
...
cal_A_range <- (max(cal.plsr.output$A)-min(cal.plsr.output$A))
cal.NRMSEP <-  round((sqrt(mean(cal.plsr.output$PLSR_CV_Residuals^2))/cal_A_range)*100,3)

val.plsr.output <- data.frame(A = val.plsr.data$A,
...
val.plsr.output <- val.plsr.output %>%
  mutate(PLSR_Residuals = PLSR_Predicted-A)
...
val_A_range <- (max(val.plsr.output$A)-min(val.plsr.output$A))
val.NRMSEP <- round((sqrt(mean(val.plsr.output$PLSR_Residuals^2))/val_A_range)*100,3)
```
...
plot(cal.plsr.output.N1$PLSR_CV_Predicted, cal.plsr.output.N1[,"A"], 
       lwd=1.6, xlim=c(rng_vals[1], rng_vals[2]), ylim=c(rng_vals[1], rng_vals[2]), 
       pch=21, col="black", bg = tCol[1], cex=2, 
       xlab=expression("CV Predicted A ("~mu*mol*m^-2*s^-1*")"),
       ylab=expression("Measured A ("~mu*mol*m^-2*s^-1*")"),
       cex.axis=3, cex.lab = 3, font.lab = 2, font = 1)
points(cal.plsr.output.N2$PLSR_CV_Predicted, cal.plsr.output.N2[,"A"], 
...
#dev.off()
```
Figure 4: Measured values  versus cross-validation values in calibration dataset (*n* = 74) of A ($\mu molm^{-2}s^{-1}$) .   
...
```
Figure 5: Residuals from the cross validation values versus cross validation predicted values (*n* = 74) of A ($\mu molm^{-2}s^{-1}$)   
...
par(mfrow=c(1,1), mar=c(6, 11, 1, 0.4), oma=c(0, 0.1, 0, 0.2), mgp = c(5, 2, 0)) 
plotrix::plotCI(val.plsr.output.N1$PLSR_Predicted, val.plsr.output.N1[,"A"], 
...
plotrix::plotCI(val.plsr.output.N2$PLSR_Predicted, val.plsr.output.N2[,"A"], add = T,
...
#dev.off()
```
Figure 6: Measured values versus prediction in validation dataset (*n* = 20) of A ($\mu molm^{-2}s^{-1}$). Error bars represent 95%CIs.  
...
```
Figure 7: Residuals from the external validation values versus predicted values (*n* = 20) of A ($\mu molm^{-2}s^{-1}$)     

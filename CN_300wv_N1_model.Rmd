---
title: "Use side-view HSI data to predict CN ratio from N1 (high-level nitrogen) data"
output: pdf_document
---
...
file_path <- "./results/reliefF_plsr/sideviewA/CN_N1_model"
```
...
cal.ID <- y_metadata %>% select(ID, Treatment) %>%
  filter(Treatment == 'N1') %>% select(ID) %>% as.matrix() %>% as.vector()
...
cal_ref_95perc = ggplot()+
  ...
  labs(title = "CN N1 calibration dataset", x= "Wavelength (nm)", y = "Reflectance (%)") +
  theme_bw(base_size=18) +
  theme(
...
val_ref_95perc = ggplot()+
  ...
  scale_y_continuous(breaks = seq(Treatment_sel_wv_ref_min, 
                     ...
                     labels = round(seq(Treatment_sel_wv_ref_min, 
                                        Treatment_sel_wv_ref_max, length.out = 3), 3),
                     limits = c(Treatment_sel_wv_ref_min, Treatment_sel_wv_ref_max)) +
  labs(title = "CN N1 validation dataset", x= "Wavelength (nm)", y = "Reflectance (%)") +
  theme_bw(base_size=18) +
  theme(
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
...
maxComps <- 15
#pdf(file = paste0(file_path, "/CN_N1_nComps.pdf"), bg = "white", width = 7, height = 5)
...
ggsave(filename = paste0(file_path, "/CN_N1_cal_scatter_plot.pdf"), 
       plot = cal_scatter_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/CN_N1_cal_resid_plot.pdf"), 
       plot = cal_resid_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/CN_N1_val_CI_plot.pdf"), 
       plot = val_CI_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/CN_N1_val_PI_plot.pdf"), 
       plot = val_PI_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/CN_N1_val_resid_plot.pdf"), 
       plot = val_resid_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/CN_N1_selected_wv.pdf"), 
       plot = selected_wv, 
       device = "pdf", width = 7, height = 5, units = "cm", dpi = 400)

write.csv(x=reliefF, 
          file = paste0(file_path, "/CN_N1_reliefF_300.csv"), 
          row.names = F)
write.csv(x=cal.plsr.output, 
          file = paste0(file_path, "/CN_N1_cal.plsr.output.csv"), 
          row.names = F)
write.csv(x=val.plsr.output, 
          file = paste0(file_path, "/CN_N1_val.plsr.output.csv"), 
          row.names = F)
write.csv(x=out.jk.coefs, 
          file = paste0(file_path, "/CN_N1_out.jk.coefs.csv"), 
          row.names = F)
write.csv(x=coefs, 
          file = paste0(file_path, "/CN_N1_coefs.csv"), 
          row.names = T)
```

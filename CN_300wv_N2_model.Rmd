---
title: "Use side-view HSI data to predict CN ratio from N2 (low-level nitrogen) data"
output: pdf_document
---
...
rm(list = ls())
## Load package 
list.of.packages <- c("dplyr", "tidyverse", "reshape2", "ggplot2", "pls", "scales", "kableExtra", "ggpubr", "gridExtra", "CORElearn", "spectratrait")
...
file_path <- "./results/reliefF_plsr/sideviewA/CN_N2_model"
```

```{r select samples based on treatment, echo = F, fig.align='center', fig.width = 4, fig.height = 4}
trait <- phyraw %>%
  select("ID", "CN", "Treatment", "Subpop") %>%
  na.omit() 
table(trait[,"Treatment"]) #N1 = N2 = 44
...
trait_summary <- trait %>% group_by(Treatment) %>% 
  summarise(mean = mean(CN), 
            lo = t.test(CN, conf.level = 0.95)$conf.int[1],
            hi = t.test(CN, conf.level = 0.95)$conf.int[2]) %>%
  as.data.frame()
```

```{r compare CN in two treatments, echo = F, fig.align='center', fig.width = 4, fig.height = 4}
# color settings
subpop_col <- c("IND" = "red", "TRJ" = "blue")
treatment_shape = c('N1' = 16, 'N2' = 1)

strip_chart <- ggplot(aes(x=Treatment, y=CN), data = trait) +
  geom_jitter(aes(shape = Treatment), color = "grey", width = 0.2, size = 2)+
  geom_errorbar(data = trait_summary, 
                mapping = aes(x=Treatment, y = NULL, ymin=lo, ymax=hi),
                show.legend = F, width = 0.1) +
  geom_point(data = trait_summary, aes(x=Treatment, y=mean, shape = Treatment), 
             show.legend = F, size = 4) +  
  scale_shape_manual(values = treatment_shape)+
  theme_bw(base_size=18,
           )+
  labs(x=NULL, y = "CN ratio")
strip_chart
``` 
...
min_selectNcomp <- function(object, ncomp = maxComps, plot = FALSE, ...) {
    ...
    rmseps <- c(RMSEP(object, "CV")$val) # rmsep for components ranging from 1 to sample size-1
    maxIdx <- ncomp + 1
    absBest <- which.min(rmseps[seq_len(maxIdx)]) # select min rmsep based on maxIdx considered
    selection <- absBest -1
...
cal.ID <- y_metadata %>% select(ID, Treatment) %>%
  filter(Treatment == 'N2') %>% select(ID) %>% as.matrix() %>% as.vector()
...
reliefF_star = reliefF[order(reliefF$value, decreasing = T)[1:300],]
tail(reliefF_star)
```

```{r cal and val datasets, echo = F, fig.align='center', fig.width = 9, fig.height = 5}
...
Treatment_sel_wv_ref_min = range(cal.plsr.long$value, val.plsr.long$value)[1]
Treatment_sel_wv_ref_max = range(cal.plsr.long$value, val.plsr.long$value)[2]
...
cal_ref_95perc = ggplot()+
  ...
  scale_y_continuous(breaks = seq(Treatment_sel_wv_ref_min, 
                                  Treatment_sel_wv_ref_max, length.out = 3), 
                     labels = round(seq(Treatment_sel_wv_ref_min, 
                                        Treatment_sel_wv_ref_max, length.out = 3), 3),
                     limits = c(Treatment_sel_wv_ref_min, Treatment_sel_wv_ref_max)) +
  labs(title = "CN N2 calibration dataset", x= "Wavelength (nm)", y = "Reflectance (%)") +
  theme_bw(base_size=18) +
  theme(
...
val_ref_95perc = ggplot()+
  ...
  scale_y_continuous(breaks = seq(Treatment_sel_wv_ref_min, 
                     ...
                     labels = round(seq(Treatment_sel_wv_ref_min, 
                                        Treatment_sel_wv_ref_max, length.out = 3), 3),
                     limits = c(Treatment_sel_wv_ref_min, Treatment_sel_wv_ref_max)) +
  labs(title = "CN N2 validation dataset", x= "Wavelength (nm)", y = "Reflectance (%)") +
  theme_bw(base_size=18) +
  theme(
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
...
Treatment_ref_95perc = ggarrange(cal_ref_95perc, val_ref_95perc, nrow = 2)
Treatment_ref_95perc
```

```{r selected wv, echo=FALSE}
full_spectra_cal_ID = melt(X_IDdata[X_IDdata$ID %in% cal.ID, -1]) #remove ID
full_spectra_cal_ID_summary = full_spectra_cal_ID %>%
  group_by(variable) %>%
...
selected_wv = ggplot()+
  ...
  geom_ribbon(aes(x = wv, ymin = lo, ymax=hi), 
              fill = 'grey70', data = full_spectra_cal_ID_summary) +
  geom_line(aes(x = wv, y = mean), data = full_spectra_cal_ID_summary) +
  theme_bw()+
  theme(
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
...
maxComps <- 10
#pdf(file = paste0(file_path, "/CN_N2_nComps.pdf"), bg = "white", width = 7, height = 5)
par(mfrow=c(1,1), mar=c(6,7,1,0.5), oma=c(0, 0.1, 0, 0.2), mgp = c(3, 1, 0))
nComps = min_selectNcomp(plsr.ini,plot = T, ncomp = maxComps, 
...
coefs = jackknife[-1,,,]
pred_jackknife = matrix(rep(intercepts, length(val.plsr.data[,'CN'])), 
                              byrow = T,
...
rng = range(val.plsr.output$upi, val.plsr.output$lpi,
            ...
            cal.plsr.output$Observed, cal.plsr.output$Predicted, 30)
...
ggsave(filename = paste0(file_path, "/CN_N2_cal_scatter_plot.pdf"), 
       plot = cal_scatter_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/CN_N2_cal_resid_plot.pdf"), 
       plot = cal_resid_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/CN_N2_val_CI_plot.pdf"), 
       plot = val_CI_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/CN_N2_val_PI_plot.pdf"), 
       plot = val_PI_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/CN_N2_val_resid_plot.pdf"), 
       plot = val_resid_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/CN_N2_selected_wv.pdf"), 
       plot = selected_wv, 
       device = "pdf", width = 7, height = 5, units = "cm", dpi = 400)

write.csv(x=reliefF, 
          file = paste0(file_path, "/CN_N2_reliefF_300.csv"), 
          row.names = F)
write.csv(x=cal.plsr.output, 
          file = paste0(file_path, "/CN_N2_cal.plsr.output.csv"), 
          row.names = F)
write.csv(x=val.plsr.output, 
          file = paste0(file_path, "/CN_N2_val.plsr.output.csv"), 
          row.names = F)
write.csv(x=out.jk.coefs, 
          file = paste0(file_path, "/CN_N2_out.jk.coefs.csv"), 
          row.names = F)
write.csv(x=coefs, 
          file = paste0(file_path, "/CN_N2_coefs.csv"), 
          row.names = T)
```

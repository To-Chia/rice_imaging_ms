---
title: "Use side-view HSI data to predict CN ratio from Tropical japonica data"
output: pdf_document
---
...
file_path <- "./results/reliefF_plsr/sideviewA/CN_TRJ_model"
```

```{r select samples based on subpop, echo = F, fig.align='center', fig.width = 4, fig.height = 4}
trait <- phyraw %>%
  select("ID", "CN", "Treatment", "Subpop") %>%
  na.omit() 
table(trait[,"Subpop"]) #IND = 57, TRJ = 31
...
trait_summary <- trait %>% group_by(Subpop) %>% 
...
```{r compare CN in two subpops, echo = F, fig.align='center', fig.width = 4, fig.height = 4}
subpop_col <- c("IND" = "red", "TRJ" = "blue")

strip_chart <- ggplot(aes(x=Subpop, y=CN), data = trait) +
  geom_jitter(color = "grey", width = 0.2, size = 2)+
  geom_errorbar(data = trait_summary, 
                mapping = aes(x=Subpop, y = NULL, ymin=lo, ymax=hi, col = Subpop),
                show.legend = F, width = 0.1) +
  scale_color_manual(values = subpop_col)+
  geom_point(data = trait_summary, aes(x=Subpop, y=mean, fill = Subpop), shape = 24, #triangle 
             show.legend = F, size = 4) +  
  scale_fill_manual(values = subpop_col)+
...
cal.ID <- y_metadata %>% select(ID, Subpop) %>%
  filter(Subpop == 'TRJ') %>% select(ID) %>% as.matrix() %>% as.vector()
...
Subpop_sel_wv_ref_min = range(cal.plsr.long$value, val.plsr.long$value)[1]
Subpop_sel_wv_ref_max = range(cal.plsr.long$value, val.plsr.long$value)[2]
...
cal_ref_95perc = ggplot()+
  ...
  scale_y_continuous(breaks = seq(Subpop_sel_wv_ref_min, 
                                  Subpop_sel_wv_ref_max, length.out = 3), 
                     labels = round(seq(Subpop_sel_wv_ref_min, 
                                        Subpop_sel_wv_ref_max, length.out = 3), 3),
                     limits = c(Subpop_sel_wv_ref_min, Subpop_sel_wv_ref_max)) +
  labs(title = "CN TRJ calibration dataset", x= "Wavelength (nm)", y = "Reflectance (%)") +
  theme_bw(base_size=18) +
  theme(
...
val_ref_95perc = ggplot()+
  ...
  scale_y_continuous(breaks = seq(Subpop_sel_wv_ref_min, 
                     ...
                     labels = round(seq(Subpop_sel_wv_ref_min, 
                                        Subpop_sel_wv_ref_max, length.out = 3), 3),
                     limits = c(Subpop_sel_wv_ref_min, Subpop_sel_wv_ref_max)) +
  labs(title = "CN TRJ validation dataset", x= "Wavelength (nm)", y = "Reflectance (%)") +
  theme_bw(base_size=18) +
  theme(
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
...
Subpop_ref_95perc = ggarrange(cal_ref_95perc, val_ref_95perc, nrow = 2)
Subpop_ref_95perc
```
...
selected_wv = ggplot()+
  geom_vline(aes(xintercept=variable), data=cal.plsr.long, color = "blue") + 
...
maxComps <- 15
#pdf(file = paste0(file_path, "/CN_TRJ_nComps.pdf"), bg = "white", width = 7, height = 5)
...
ggsave(filename = paste0(file_path, "/CN_TRJ_cal_scatter_plot.pdf"), 
       plot = cal_scatter_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/CN_TRJ_cal_resid_plot.pdf"), 
       plot = cal_resid_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/CN_TRJ_val_CI_plot.pdf"), 
       plot = val_CI_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/CN_TRJ_val_PI_plot.pdf"), 
       plot = val_PI_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/CN_TRJ_val_resid_plot.pdf"), 
       plot = val_resid_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/CN_TRJ_selected_wv.pdf"), 
       plot = selected_wv, 
       device = "pdf", width = 7, height = 5, units = "cm", dpi = 400)

write.csv(x=reliefF, 
          file = paste0(file_path, "/CN_TRJ_reliefF_300.csv"), 
          row.names = F)
write.csv(x=cal.plsr.output, 
          file = paste0(file_path, "/CN.TRJ.cal.plsr.output.csv"), 
          row.names = F)
write.csv(x=val.plsr.output, 
          file = paste0(file_path, "/CN.TRJ.val.plsr.output.csv"), 
          row.names = F)
write.csv(x=out.jk.coefs, 
          file = paste0(file_path, "/CN_TRJ_out.jk.coefs.csv"), 
          row.names = F)
write.csv(x=coefs, 
          file = paste0(file_path, "/CN_TRJ_coefs.csv"), 
          row.names = T)
```

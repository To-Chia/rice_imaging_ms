---
title: "Use side-view HSI data to predict CN ratio from Indica data"
output: pdf_document
---
...
file_path <- "./results/reliefF_plsr/sideviewA/CN_IND_model"
```
...
strip_chart <- ggplot(aes(x=Subpop, y=CN), data = trait) +
  ...
  geom_errorbar(data = trait_summary, mapping = aes(x=Subpop, y = NULL, ymin=lo, ymax=hi, col = Subpop),
...
cal.ID <- y_metadata %>% select(ID, Subpop) %>%
  filter(Subpop == 'IND') %>% select(ID) %>% as.matrix() %>% as.vector()
...
cal_ref_95perc = ggplot()+
  ...
  labs(title = "CN IND calibration dataset", x= "Wavelength (nm)", y = "Reflectance (%)") +
  theme_bw(base_size=18) +
  theme(
...
val_ref_95perc = ggplot()+
  ...
  scale_y_continuous(breaks = seq(Subpop_sel_wv_ref_min, 
                     ...
                     labels = round(seq(Subpop_sel_wv_ref_min, 
                                        Subpop_sel_wv_ref_max, length.out = 3), 3),
                     limits = c(Subpop_sel_wv_ref_min, Subpop_sel_wv_ref_max)) +
  labs(title = "CN IND validation dataset", x= "Wavelength (nm)", y = "Reflectance (%)") +
  theme_bw(base_size=18) +
  theme(
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
...
selected_wv = ggplot()+
  geom_vline(aes(xintercept=variable), data=cal.plsr.long, color = "red") + 
...
maxComps <- 15
pdf(file = paste0(file_path, "/CN_IND_nComps.pdf"), bg = "white", width = 7, height = 5)
...
ggsave(filename = paste0(file_path, "/CN_IND_cal_scatter_plot.pdf"), 
       plot = cal_scatter_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/CN_IND_cal_resid_plot.pdf"), 
       plot = cal_resid_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/CN_IND_val_CI_plot.pdf"), 
       plot = val_CI_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/CN_IND_val_PI_plot.pdf"), 
       plot = val_PI_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/CN_IND_val_resid_plot.pdf"), 
       plot = val_resid_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/CN_IND_selected_wv.pdf"), 
       plot = selected_wv, 
       device = "pdf", width = 7, height = 5, units = "cm", dpi = 400)

write.csv(x=reliefF, 
          file = paste0(file_path, "/CN_IND_reliefF_300.csv"), 
          row.names = F)
write.csv(x=cal.plsr.output, 
          file = paste0(file_path, "/CN.IND.cal.plsr.output.csv"), 
          row.names = F)
write.csv(x=val.plsr.output, 
          file = paste0(file_path, "/CN.IND.val.plsr.output.csv"), 
          row.names = F)
write.csv(x=out.jk.coefs, 
          file = paste0(file_path, "/CN_IND_out.jk.coefs.csv"), 
          row.names = F)
write.csv(x=coefs, 
          file = paste0(file_path, "/CN_IND_coefs.csv"), 
          row.names = T)
```

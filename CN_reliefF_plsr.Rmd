...
Quantify leaf-level CN ratio (CN) at mid-tillering stage with hyperspectral imaging data (HSI data).    
...
2. Build a PLSR model with $W_{PLSR}$ as input and CN as output.  
...
inVar <- "CN"
file_path <- "./results/reliefF_plsr/CN/"
```
...
ref_trait_expRank <- attrEval(CN ~., ref_trait_num, estimator = "RReliefFexpRank", ReliefIterations = nrow(trait)*50)
...
```
Figure 1: Attributes values of HSI data on Week 9 for CN ratio. Wavelengths with the top 300 attribute values are considered as important (red, $W_{PLSR}$) and are model inputs.  
```{r, echo=FALSE, eval=FALSE}
ggsave(filename = paste0(file_path, "CN_ratio_expRank.png"), plot = ref_trait_expRank_plot, device="png", width = 30, height = 12, units = "cm", dpi = 400)
...
CN_range <- range(trait$CN)
#sd(trait$CN)
cal_hist_plot <-ggplot(aes(x = CN), data = cal.plsr.data) +
  geom_histogram(binwidth =1.7, fill=I("#0072B2"), col=I("black")) +
  coord_cartesian(xlim = c(10, 30)) +
  theme_bw(base_size = 18) +
  labs(x = "CN ratio", y = "Count") +
  ggtitle("Cal. (n = 70)") 

val_hist_plot <- ggplot(aes(x = CN), data = val.plsr.data) +
  geom_histogram(binwidth = 2.3, fill=I("#0072B2"), col=I("black")) +
  coord_cartesian(xlim = c(10, 30)) +
  theme_bw(base_size = 18) +
  labs(x = "CN ratio", y = "Count") +
  ggtitle("Val. (n = 18)")
...
cal_val_histograms 
```
Figure 2: Frequency distributions of CN ratio in the calibration (left, *n* = 70) and validation (right, *n* = 18) datasets. 
...
#png(filename = paste0(file_path, "CN_nComps.png"), bg = "white", width = 7, height = 5, units = "in", res = 400)
...
#calibration
cal.plsr.output <- data.frame(CN = cal.plsr.data$CN,
                              PLSR_Predicted=fit,
                              PLSR_CV_Predicted=as.vector(plsr.out$validation$pred[,,nComps]))
cal.plsr.output <- cal.plsr.output %>%
  mutate(PLSR_CV_Residuals = PLSR_CV_Predicted-CN)
...
cal_CN_range <- (max(cal.plsr.output$CN)-min(cal.plsr.output$CN))
cal.NRMSEP <-  round((sqrt(mean(cal.plsr.output$PLSR_CV_Residuals^2))/cal_CN_range)*100,3)

val.plsr.output <- data.frame(CN = val.plsr.data$CN,
                              PLSR_Predicted=as.vector(predict(plsr.out, 
                                                               newdata = val.plsr.data, 
                                                               ncomp=nComps, 
                                                               type="response")[,,]))
val.plsr.output <- val.plsr.output %>%
  mutate(PLSR_Residuals = PLSR_Predicted-CN)
...
val_CN_range <- (max(val.plsr.output$CN)-min(val.plsr.output$CN))
val.NRMSEP <- round((sqrt(mean(val.plsr.output$PLSR_Residuals^2))/val_CN_range)*100,3)
```

```{r cal scatter, echo = F, fig.align='center', fig.width = 9, fig.height = 5}
rng <- range(cal.plsr.output$CN, cal.plsr.output$PLSR_CV_Predicted,
             val.plsr.output$CN, val.plsr.output$PLSR_Predicted)
text_xloc <- seq(round(rng[1]), round(rng[2]), length = 10)[3]
text_y_R2 <- (round(rng[2])-round(rng[1]))/10*9+round(rng[1])
text_y_RMSEP <- (round(rng[2])-round(rng[1]))/10*8+round(rng[1])
text_y_NRMSEP <- (round(rng[2])-round(rng[1]))/10*7+round(rng[1])

cal_scatter_plot <- ggplot(cal.plsr.output, aes(x=CN, y=PLSR_CV_Predicted)) + 
  theme_bw() + geom_point() + 
 ...
 annotate("text", size = 7 , x= text_xloc, y = c(text_y_R2, text_y_RMSEP), parse = TRUE, 
  ...
  annotate("text", size = 7 , x= text_xloc, y = text_y_NRMSEP, label = "%RMSEP = 11.63") +
  annotate("text", x = text_xloc, y = 27.5, parse = T, label = "Cal. (italic(n) == 70)", size = 7)
```
...
cal_resid <- ggplot(cal.plsr.output, aes(x = CN, y = PLSR_CV_Residuals)) +
  geom_point() + 
  geom_hline(yintercept = 0, color="black", 
             linetype="dashed", size=1) + 
  theme_bw(base_size = 18) +
 labs(title = "Cal. (n = 70)", x = "\n Measured: CN ratio", y = "CV residuals")
```

```{r export cal resid, echo = F, eval = F, fig.align='center', fig.width = 5, fig.height = 5}
cal_resid
ggsave(filename = paste0(file_path, "CN_Cal_Residuals.png"), plot = cal_resid, device="png", width = 12, height = 12, units = "cm", dpi = 300)
```

```{r val scatter, echo = F, fig.align='center', fig.width = 5, fig.height = 5}
val_scatter_plot <- ggplot(val.plsr.output, aes(x=CN, y=PLSR_Predicted)) + 
  theme_bw() + geom_point() + 
  geom_abline(intercept = 0, slope = 1, color="dark grey", linetype="dashed", size=1.5) + 
  geom_smooth(method = "lm", formula = y ~ x, se = F) +
  scale_x_continuous(breaks = seq(round(rng[1]), round(rng[2]), length = 5), 
  ...
  theme(axis.text=element_text(size=25, face = "bold"), legend.position="none",
   ...
   annotate("text", size = 7 , x= text_xloc, y = text_y_NRMSEP, label = "%RMSEP = 14.39") +
   annotate("text", x = text_xloc, y = 27.5, parse = T, label = "Val. (italic(n) == 18)", size = 7 )
```
...
val_resid <- ggplot(val.plsr.output, aes(x = CN, y=PLSR_Residuals)) +
  geom_point() + 
  geom_hline(yintercept = 0, color="black", 
             linetype="dashed", size=1) + theme_bw(base_size = 18) + 
  labs(title = "Val. (n = 18)", x = "\n Measured: CN ratio", y = "Residuals")
```

```{r export val resid, echo = F, eval = F, fig.align='center', fig.width = 5, fig.height = 5}
val_resid
ggsave(filename = paste0(file_path, "CN_Val_Residuals.png"), plot = val_resid, device="png", width = 12, height = 12, units = "cm", dpi = 300)
```
...
```
Figure 4: Cross-validated values versus measured values (left, *n* = 70) and external validation values versus measured values (right, *n* = 18) of CN ratio
```{r resid plots, echo = F, fig.align='center', fig.width = 8, fig.height = 4}
residsplots <-  grid.arrange(cal_resid, val_resid, ncol=2)
```
Figure 5: Residuals from the cross-validated values versus measured values (left, *n* = 70) and residuals from the external validation values versus measured values (right, *n* = 18) of CN ratio  

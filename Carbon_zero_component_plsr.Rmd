...
Predict carbon (%, *n* = 88, N1=N2=44) with hyperspectral data.  

# Methods  

1. Use RReliefFexpRank algorithm to select the top 300 wavelengths that were important for carbon.  
2. Conduct PCA on the reflectances of the selected wavelengths to concentrate the variance.  
3. Use PCs' scores as input variables to build a PLSR model.  
...
inVar <- "C"
file_path <- "./results/relieF_pca_plsr/carbon/"
...
wv_trait_expRank <- attrEval(C ~., wv_trait_2, estimator = "RReliefFexpRank", ReliefIterations = nrow(trait)*50) 
...
```
Figure 1: Attribute values of HSI data on Week 9 for carbon (%). Wavelengths with the top 300 attribute values are considered as important (red). 
```{r, include = FALSE}
...
HSI_p
#ggsave(filename = paste0(file_path, "/Carbon_sel_300wv.png"), plot = HSI_p, device="png", width = 30, height = 12, units = "cm", dpi = 300)
...
sum(HSI_var_percent[1:3]) #95.1
```

```{r, echo = F, warnings = F, fig.align='center', fig.width = 4, fig.height = 4}
#png(filename = paste0(file_path, "/HSI_Carbon_var_percent.png"), bg = "white", width = 10, height = 10, units = "in", res = 300)
...
#dev.off()
```
Figure 2: PCA scree plot. The first three PCs accounted for 95.1% of the variance. The top ten loadings in PC1, PC2 and PC3 are wavelengths from 856 to 941 nm, from 706 to 717 nm and from 2279 to 2497 nm, respectively.  
```{r, include=FALSE}
...
HSI_top_loadings <- data.frame(Trait = "Carbon", PC = rep(c("PC1", "PC2", "PC3"), each = 10), 
...
#write.csv(file = paste0(file_path, "/HSI_Carbon_top_loadings.csv"), x= HSI_top_loadings_2, row.names = F)
```
...
The data were first grouped by treatment. Then, 80% of the data in each treatment were sampled for the calibration dataset.  
```{r, echo = F, fig.align='center', fig.width = 9, fig.height = 5}
HSI_I <- as.matrix(HSI_scores)
plsr_data <- data.frame(trait, input_scores = I(HSI_I))
set.seed(444)
prop <- 0.8
...
val_hist_plot <- qplot(val.plsr.data$C, geom="histogram", bins =4,
                       main = "Val. (n = 18) ",
                       xlab = "Carbon (%)", ylab = "Count", 
                       fill=I("#0072B2"),col=I("black")) + theme_bw(base_size = 18)
#val_hist_plot
...
```
Figure 3: Frequency distributions of carbon (%) in the calibration (left, *n* = 70) and evaluation (right, *n* = 18) datasets. 
\newpage  

2. Fit a plsr model    
Model: Carbon (%) = Scores of the PCs. The scores were not scaled. 
The model was cross-validated with leave-one-out method.  
```{r, echo = F, fig.align='center', fig.width = 9, fig.height = 5}
plsr.ini <- plsr(C~input_scores, scale=FALSE,
                 validation="LOO",trace=FALSE,data=cal.plsr.data)
#png(filename = paste0(file_path, "Carbon_nComps.png"), bg = "white", width = 10, height = 10, units = "in", res = 300)
par(mar = c(6, 7, 4, 2), mgp = c(5, 2, 0))
nComp <- selectNcomp(plsr.ini, method = "onesigma", plot = T, 
                     cex.axis = 2, cex.lab = 2, xlim = c(0, 15), ylim = c(1.5, 3.5),
                     font.lab = 2, font = 2)
#dev.off()
```
Figure 4: Root mean square error of prediction (RMSEP) as a function of model component number in PLSR models. The dotted black vertical line was the number of components that has the lowest RMSEP. The vertical long-dashed blue line indicates the suggested number of components. In this case, they overlap. The points and error bars are the means and standard errors of RMSEP in the cross validation, respectively.   
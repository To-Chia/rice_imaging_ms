---
title: "Predict Jmax from hyperspectral data.  "
date: "`r Sys.Date ()`"
output: pdf_document
---
...
2. Built a PLSR model with $W_{PLSR}$ as input and Jmax ($\mu molm^{-2}s^{-1}$, *n* = 89) as output. Methods were adapted from Burnett et al. (2021).  
...
inVar <- "Jmax"
file_path <- "./results/reliefF_plsr/Jmax/"
```

```{r combine phy data from W6 and W13, echo = F, fig.align='center'}
Jmax_df <- phyraw[complete.cases(phyraw$Jmax), c("ID", "Jday_start", "Treatment", "Accession", "Subpop", "Jmax")] 
head(Jmax_df)
Jmax_W9 <- Jmax_df %>%
...
Jmax_W13 <-  Jmax_df %>%
  ...
  mutate(Week = "13")
str(Jmax_W13)
Jmax_bind <- rbind(Jmax_W9, Jmax_W13)
Jmax_bind$Week <- factor(Jmax_bind$Week, levels = c('9', "13"))

Jmax_hist <- ggplot(aes(x=Jmax, fill=Week), data = Jmax_bind) +
    geom_histogram( color="#e9ecef", alpha=0.6, position = 'identity', bins = 25) +
    scale_fill_manual(values=c("#69b3a2", "#404080")) + 
    labs(x = "Jmax") + theme_bw()
Jmax_hist 
```
Figure 1. Jmax ($\mu molm^{-2}s^{-1}$, *n* = 89) of each rice plants on weeks 9 (green)and 13 (purple).  
```{r combine HSI data between W6 and W13, echo = F, fig.align='center'}
ref_W9_meta <- ref_W9_meta[ref_W9_meta$ID %in% Jmax_W9$ID, -c(746:762)] #drop columns with wv > 2500
...
ref_W13_meta <- ref_W13_meta[ref_W13_meta$ID %in% Jmax_W13$ID, -c(746:762)]
...
Jmax_W9$ID <- paste(Jmax_W9$ID, "_W9", sep='')
Jmax_W13$ID <- paste(Jmax_W13$ID, "_W13", sep='')
trait_bind <- rbind(Jmax_W9, Jmax_W13)

ref_trait <- merge(ref, select(trait_bind, c(ID, Jmax)), by = "ID")
ref_trait_num <- ref_trait[ ,-1] # drop ID

ref_trait_expRank <- attrEval(Jmax ~., ref_trait_num, estimator = "RReliefFexpRank", ReliefIterations = nrow(trait_bind)*50) 
...
Jmax_feature_plot
```
Figure 3: Attributes values of HSI data on Weeks 9 and 13 for Jmax ($\mu molm^{-2}s^{-1}$, *n* = 89). Wavelengths with the top 300 attribute values are considered as important (red, $W_{PLSR}$) and are used for model input. 

```{r att anotate output, echo=FALSE, eval = F, fig.align='center', fig.width = 10, fig.height = 4}
Jmax_feature_plot
ggsave(filename = paste0(file_path, "Jmax_feature.png"), plot = E_feature_plot, device = "png", width = 30, height = 12, units = "cm", dpi = 400)
```
...
cal_hist_plot <- qplot(cal.plsr.data$Jmax, geom="histogram", bins =10,
                       main = "Cal. (n = 70)",
                       xlab = expression("Jmax ("~mu*mol*m^-2*s^-1*")"), ylab = "Count", 
                       fill=I("#0072B2"), col=I("black")) + theme_bw(base_size = 18) +
  theme(axis.text.x = element_text(hjust = 1))

val_hist_plot <- qplot(val.plsr.data$Jmax, geom="histogram", bins =7,
                       main = "Val. (n = 19)",
                       xlab = expression("Jmax ("~mu*mol*m^-2*s^-1*")"), ylab = "Count", 
                       fill=I("#0072B2"), col=I("black")) + theme_bw(base_size = 18)
cal_val_histograms <- grid.arrange(cal_hist_plot, val_hist_plot, ncol=2)
```
Figure 4: Frequency distributions of Jmax ($\mu molm^{-2}s^{-1}$) in the calibration (*n* = 70) and validation (*n* = 19) datasets.  

2. Fitted a plsr model    

```{r determine number of components, echo = F, fig.align='center', fig.width = 9, fig.height = 5}
plsr.ini <- plsr(Jmax~Spectra, scale=FALSE,
...
cal_Jmax_range <- (max(cal.plsr.output$Jmax)-min(cal.plsr.output$Jmax))
cal.NRMSEP <-  round((sqrt(mean(cal.plsr.output$PLSR_CV_Residuals^2))/cal_Jmax_range)*100,3)

val.plsr.output <- data.frame(Jmax = val.plsr.data$Jmax,
...
val.plsr.output <- val.plsr.output %>%
  mutate(PLSR_Residuals = PLSR_Predicted-Jmax)
...
val_Jmax_range <- (max(val.plsr.output$Jmax)-min(val.plsr.output$Jmax))
val.NRMSEP <- round((sqrt(mean(val.plsr.output$PLSR_Residuals^2))/val_Jmax_range)*100,3)
```
...
rng_vals <- c(min(val.plsr.output$LCI), max(val.plsr.output$PLSR_Predicted))
tCol <- c("black", "white")
...
plot(cal.plsr.output.N1$PLSR_CV_Predicted, cal.plsr.output.N1[,"Jmax"], 
       lwd=1.6, xlim=c(rng_vals[1], rng_vals[2]), ylim=c(rng_vals[1], rng_vals[2]), 
       pch=21, col="black", bg = tCol[1], cex=2, 
       xlab=expression("CV Predicted Jmax ("~mu*mol*m^-2*s^-1*")"),
       ylab=expression("Measured Jmax ("~mu*mol*m^-2*s^-1*")"),
       cex.axis=3, cex.lab = 3, font.lab = 2, font = 2)
points(cal.plsr.output.N2$PLSR_CV_Predicted, cal.plsr.output.N2[,"Jmax"], 
...
#dev.off()
```
Figure 6: Measured values  versus cross-validation values in calibration dataset (*n* = 70) of Jmax ($\mu molm^{-2}s^{-1}$) .   
...
```
Figure 7: Residuals from the cross validation values versus cross validation predicted values (*n* = 70) of Jmax ($\mu molm^{-2}s^{-1}$)   
...
par(mfrow=c(1,1), mar=c(6, 11, 1, 0.4), oma=c(0, 0.1, 0, 0.2), mgp = c(5, 2, 0)) 
plotrix::plotCI(val.plsr.output.N1$PLSR_Predicted, val.plsr.output.N1[,"Jmax"], 
       ...
       ylab=expression("Measured Jmax ("~mu*mol*m^-2*s^-1*")"),
       cex.axis=3, cex.lab = 3, font.lab = 2, font = 2)
plotrix::plotCI(val.plsr.output.N2$PLSR_Predicted, val.plsr.output.N2[,"Jmax"], add = T,
...
#dev.off()
```
Figure 8: Measured values versus prediction in validation dataset (*n* = 19) of Jmax ($\mu molm^{-2}s^{-1}$)   . Error bars represent 95%CIs.  
...
```
Figure 9: Residuals from the external validation values versus predicted values (*n* = 19) of Jmax ($\mu molm^{-2}s^{-1}$)     

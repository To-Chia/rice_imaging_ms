---
title: "Use side-view HSI data to predict N from N2 (high-level) data"
output: pdf_document
---
...
file_path <- "./results/reliefF_plsr/sideviewA/Nitrogen_N2_model"
```
...
trait_summary <- trait %>% group_by(Treatment) %>% 
  summarise(mean = mean(N), 
            lo = t.test(N, conf.level = 0.95)$conf.int[1],
            hi = t.test(N, conf.level = 0.95)$conf.int[2]) %>%
  as.data.frame()
```

```{r compare N in two treatments, echo = F, fig.align='center', fig.width = 4, fig.height = 4}
# color settings
subpop_col <- c("IND" = "red", "TRJ" = "blue")
treatment_shape = c('N1' = 16, 'N2' = 1)

strip_chart <- ggplot(aes(x=Treatment, y=N), data = trait) +
...
cal_ref_95perc = ggplot()+
  ...
  labs(title = "N N2 calibration dataset", x= "Wavelength (nm)", y = "Reflectance (%)") +
  theme_bw(base_size=18) +
  theme(
...
val_ref_95perc = ggplot()+
  ...
  scale_y_continuous(breaks = seq(Treatment_sel_wv_ref_min, 
                     ...
                     labels = round(seq(Treatment_sel_wv_ref_min, 
                                        Treatment_sel_wv_ref_max, length.out = 3), 3),
                     limits = c(Treatment_sel_wv_ref_min, Treatment_sel_wv_ref_max)) +
  labs(title = "N N1 validation dataset", x= "Wavelength (nm)", y = "Reflectance (%)") +
  theme_bw(base_size=18) +
  theme(
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
...
#pdf(file = paste0(file_path, "/N_N2_nComps.pdf"), bg = "white", width = 7, height = 5)
...
coefs = jackknife[-1,,,]
pred_jackknife = matrix(rep(intercepts, length(val.plsr.data[,'N'])), 
                              byrow = T,
...
```{r val residual plot, echo=FALSE}
...
ggsave(filename = paste0(file_path, "/N_N2_cal_scatter_plot.pdf"), 
       plot = cal_scatter_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/N_N2_cal_resid_plot.pdf"), 
       plot = cal_resid_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/N_N2_val_CI_plot.pdf"), 
       plot = val_CI_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/N_N2_val_PI_plot.pdf"), 
       plot = val_PI_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/N_N2_val_resid_plot.pdf"), 
       plot = val_resid_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/N_N2_selected_wv.pdf"), 
       plot = selected_wv, 
       device = "pdf", width = 7, height = 5, units = "cm", dpi = 400)

write.csv(x=reliefF, 
          file = paste0(file_path, "/N_N2_reliefF_300.csv"), 
          row.names = F)
write.csv(x=cal.plsr.output, 
          file = paste0(file_path, "/N.N2.cal.plsr.output.csv"), 
          row.names = F)
write.csv(x=val.plsr.output, 
          file = paste0(file_path, "/N.N2.val.plsr.output.csv"), 
          row.names = F)
write.csv(x=out.jk.coefs, 
          file = paste0(file_path, "/N_N2_out.jk.coefs.csv"), 
          row.names = F)
write.csv(x=coefs, 
          file = paste0(file_path, "/N_N2_coefs.csv"), 
          row.names = T)
```

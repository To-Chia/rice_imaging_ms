---
title: "Use side-view HSI data to predict N"
date: "`r Sys.Date ()`"
output:
...
file_path <- "./results/reliefF_plsr/sideviewA/Nitrogen_n_400_wv"
```
...
#pdf(file = paste0(file_path, "/N_nComps.pdf"), bg = "white", width = 7, height = 5)
...
coefs = jackknife[-1,,,]
pred_jackknife = matrix(rep(intercepts, length(val.plsr.data[,'N'])), byrow = T,
...
ggsave(filename = paste0(file_path, "/N_cal_scatter_plot.eps"), 
       plot = cal_scatter_plot, 
       device = "eps", width = 5, height = 5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/N_cal_resid_plot.pdf"), 
       plot = cal_resid_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/N_val_CI_plot.eps"), 
       plot = val_CI_plot, 
       device = "eps", width = 5, height = 5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/N_val_PI_plot.pdf"), 
       plot = val_PI_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/N_val_resid_plot.pdf"), 
       plot = val_resid_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/N_selected_wv.pdf"), 
       plot = selected_wv, 
       device = "pdf", width = 7, height = 5, units = "cm", dpi = 400)

write.csv(x=reliefF, 
          file = paste0(file_path, "/N_reliefF_400.csv"), 
          row.names = F)
write.csv(x=cal.plsr.output, 
          file = paste0(file_path, "/N.cal.plsr.output.csv"), 
          row.names = F)
write.csv(x=val.plsr.output, 
          file = paste0(file_path, "/N.val.plsr.output.csv"), 
          row.names = F)
write.csv(x=out.jk.coefs, 
          file = paste0(file_path, "/N_out.jk.coefs.csv"), 
          row.names = F)
write.csv(x=coefs, 
          file = paste0(file_path, "/N_coefs.csv"), 
          row.names = T)
```

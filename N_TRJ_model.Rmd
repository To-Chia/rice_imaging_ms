---
title: "Use side-view HSI data to predict N from Tropical japonica data"
output: pdf_document
---
...
file_path <- "./results/reliefF_plsr/sideviewA/Nitrogen_TRJ_model"
```
...
```{r compare N in two subpops, echo = F, fig.align='center', fig.width = 4, fig.height = 4}
subpop_col <- c("IND" = "red", "TRJ" = "blue")

strip_chart <- ggplot(aes(x=Subpop, y=N), data = trait) +
...
cal_ref_95perc = ggplot()+
  ...
  labs(title = "N TRJ calibration dataset", x= "Wavelength (nm)", y = "Reflectance (%)") +
  theme_bw(base_size=18) +
  theme(
...
val_ref_95perc = ggplot()+
  ...
  scale_y_continuous(breaks = seq(Subpop_sel_wv_ref_min, 
                     ...
                     labels = round(seq(Subpop_sel_wv_ref_min, 
                                        Subpop_sel_wv_ref_max, length.out = 3), 3),
                     limits = c(Subpop_sel_wv_ref_min, Subpop_sel_wv_ref_max)) +
  labs(title = "N TRJ validation dataset", x= "Wavelength (nm)", y = "Reflectance (%)") +
  theme_bw(base_size=18) +
  theme(
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
...
maxComps <- 15
pdf(file = paste0(file_path, "/N_TRJ_nComps.pdf"), bg = "white", width = 7, height = 5)
...
ggsave(filename = paste0(file_path, "/N_TRJ_cal_scatter_plot.pdf"), 
       plot = cal_scatter_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/N_TRJ_cal_resid_plot.pdf"), 
       plot = cal_resid_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/N_TRJ_val_CI_plot.pdf"), 
       plot = val_CI_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/N_TRJ_val_PI_plot.pdf"), 
       plot = val_PI_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/N_TRJ_val_resid_plot.pdf"), 
       plot = val_resid_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/N_TRJ_selected_wv.pdf"), 
       plot = selected_wv, 
       device = "pdf", width = 7, height = 5, units = "cm", dpi = 400)

write.csv(x=reliefF, 
          file = paste0(file_path, "/N_TRJ_reliefF_300.csv"), 
          row.names = F)
write.csv(x=cal.plsr.output, 
          file = paste0(file_path, "/N.TRJ.cal.plsr.output.csv"), 
          row.names = F)
write.csv(x=val.plsr.output, 
          file = paste0(file_path, "/N.TRJ.val.plsr.output.csv"), 
          row.names = F)
write.csv(x=out.jk.coefs, 
          file = paste0(file_path, "/N_TRJ_out.jk.coefs.csv"), 
          row.names = F)
write.csv(x=coefs, 
          file = paste0(file_path, "/N_TRJ_coefs.csv"), 
          row.names = T)
```

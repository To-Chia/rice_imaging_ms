---
title: "Use side-view HSI data to predict SLA"
date: "`r Sys.Date ()`"
output:
...
file_path <- "./results/reliefF_plsr/sideviewA/SLA_n_300_wv"
```
...
```{r data, echo = F}
y_metadata <- phyraw %>%
  select("ID", "SLA", "Treatment", "Subpop") %>%
  na.omit() %>%
...
cal_val_list = cal_val_data_sets(cal.ID = cal.ID, val.ID = val.ID,
                                   ...
                                   X_attr = reliefF_star, 
                                   y_name = 'SLA', num_X_names = num_X_names,
                                   plot = T, y_unit = expression(" ("~cm^2*g^-1~")"))
...
cal_ref_95perc = ggplot()+
  ...
  labs(title = "SLA calibration dataset", x= "Wavelength (nm)", y = "Reflectance (%)") +
...
val_ref_95perc = ggplot()+
  ...
  scale_y_continuous(breaks = seq(sel_wv_ref_min, sel_wv_ref_max, length.out = 3), 
                     labels = round(seq(sel_wv_ref_min, sel_wv_ref_max, length.out = 3), 3),
                     limits = c(sel_wv_ref_min, sel_wv_ref_max)) +
  labs(title = "SLA validation dataset", x= "Wavelength (nm)", y = "Reflectance (%)") +
  theme_bw(base_size=18) +
  theme(
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
...
maxComps <- 15
#pdf(file = paste0(file_path, "/SLA_nComps.pdf"), bg = "white", width = 7, height = 5)
...
coefs = jackknife[-1,,,]
pred_jackknife = matrix(rep(intercepts, length(val.plsr.data[,'SLA'])), byrow = T,
...
rng = range(val.plsr.output$upi, val.plsr.output$lpi,
            ...
            cal.plsr.output$Observed, cal.plsr.output$Predicted, 180)
...
cal_scatter_plot <- ggplot(cal.plsr.output, aes(x=Observed, y=Predicted)) + 
  ...
  labs(x=expression("\n Observed: SLA ("~cm^2*g^-1~")"),
       y=expression("Predicted: SLA ("~cm^2*g^-1~")")) +
...
val_CI_plot <- ggplot(val.plsr.output, aes(x=Observed, y=Predicted)) +
  ...
  geom_point(aes(col=Subpop, shape = Treatment)) + 
  scale_shape_manual(values = treatment_shape)+
  geom_errorbar(aes(ymin = lci, ymax = uci,col=Subpop), width=10, size = 0.1) +
  scale_color_manual(values = subpop_col)+
  #geom_smooth(method = "lm", formula = y ~ x, se = F) +
  scale_x_continuous(breaks = seq(round(rng[1]), round(rng[2]), length = 5), 
                     labels = seq(round(rng[1]), round(rng[2]), length = 5), 
...
val_PI_plot <- ggplot(val.plsr.output, aes(x=Observed, y=Predicted)) +
  ...
  geom_point(aes(col=Subpop, shape = Treatment)) + 
  scale_shape_manual(values = treatment_shape)+
  geom_errorbar(aes(ymin = lpi, ymax = upi, col=Subpop), width=10, size = 0.1) +
  scale_color_manual(values = subpop_col)+
  #geom_smooth(method = "lm", formula = y ~ x, se = F) +
  scale_x_continuous(breaks = seq(round(rng[1]), round(rng[2]), length = 5), 
                     labels = seq(round(rng[1]), round(rng[2]), length = 5), 
...
ggsave(filename = paste0(file_path, "/SLA_cal_scatter_plot.pdf"), 
       plot = cal_scatter_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/SLA_cal_resid_plot.pdf"), 
       plot = cal_resid_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/SLA_val_CI_plot.pdf"), 
       plot = val_CI_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/SLA_val_PI_plot.pdf"), 
       plot = val_PI_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/SLA_val_resid_plot.pdf"), 
       plot = val_resid_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/SLA_selected_wv.pdf"), 
       plot = selected_wv, 
       device = "pdf", width = 7, height = 5, units = "cm", dpi = 400)

write.csv(x=reliefF, 
          file = paste0(file_path, "/SLA_reliefF_300.csv"), 
          row.names = F)
write.csv(x=cal.plsr.output, 
          file = paste0(file_path, "/SLA_cal.plsr.output.csv"), 
          row.names = F)
write.csv(x=val.plsr.output, 
          file = paste0(file_path, "/SLA_val.plsr.output.csv"), 
          row.names = F)
write.csv(x=out.jk.coefs, 
          file = paste0(file_path, "/SLA_out.jk.coefs.csv"), 
          row.names = F)
write.csv(x=coefs, 
          file = paste0(file_path, "/SLA_coefs.csv"), 
          row.names = T)
```

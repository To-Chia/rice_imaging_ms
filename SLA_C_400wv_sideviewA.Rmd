---
title: "Use side-view HSI data to predict SLA_C"
date: "`r Sys.Date ()`"
output:
...
file_path <- "./results/reliefF_plsr/sideviewA/SLA_C_n_400_wv"
```
...
```{r SLA_C data, echo = F}
y_metadata <- phyraw %>%
  select("ID", "SLA_C", "Treatment", "Subpop") %>%
...
```{r SLA_C feature selection, echo=FALSE}
pre_cal = input_for_reliefF(X_IDdata = X_IDdata, y_metadata = y_metadata, 
                            cal.ID = cal.ID, y_name = 'SLA_C', 
                            num_X_names = num_X_names)

reliefF = reliefF_algorithm(pre_cal = pre_cal, 
                                y_name = 'SLA_C', num_X_names = num_X_names) 
reliefF_star =  reliefF[order(reliefF$value, decreasing = T)[1:400], ] 
tail(reliefF_star)
```

```{r SLA_C cal and vel data sets, echo = F, fig.align='center', fig.width = 9, fig.height = 5}
cal_val_list = cal_val_data_sets(cal.ID = cal.ID, val.ID = val.ID,
                                   X_IDdata = X_IDdata, y_metadata = y_metadata, 
                                   X_attr = reliefF_star, 
                                   y_name = 'SLA_C', num_X_names = num_X_names,
                                   plot = T, y_unit = expression(" ("*cm^2~mg^-1*"C)"))

cal.plsr.data = cal_val_list[[1]]
val.plsr.data = cal_val_list[[2]]
```

```{r SLA_C spectra in cal and val data sets, echo=FALSE, fig.align='center', fig.width = 9, fig.height = 6}
...
cal_ref_95perc = ggplot()+
  ...
  labs(title = "SLA_C calibration dataset", x= "Wavelength (nm)", y = "Reflectance (%)") +
...
val_ref_95perc = ggplot()+
  ...
  scale_y_continuous(breaks = seq(sel_wv_ref_min, sel_wv_ref_max, length.out = 3), 
                     labels = round(seq(sel_wv_ref_min, sel_wv_ref_max, length.out = 3), 3),
                     limits = c(sel_wv_ref_min, sel_wv_ref_max)) +
  labs(title = "SLA_C validation dataset", x= "Wavelength (nm)", y = "Reflectance (%)") +
  theme_bw(base_size=18) +
  theme(
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
...
```{r SLA_C selected wv, echo=FALSE}
...
```{r SLA_C number of components CV, echo = F, fig.align='center', fig.width = 4, fig.height = 5}
plsr.ini <- plsr(SLA_C~Spectra, scale=FALSE,
                 validation="LOO",trace=FALSE,data=cal.plsr.data)
maxComps <- 15
#pdf(file = paste0(file_path, "/SLA_C_nComps.pdf"), bg = "white", width = 7, height = 5)
...
```{r SLA_C jackknife coef, echo=FALSE}
jackknife = spectratrait::f.coef.valid(plsr.out, 
                                             cal.plsr.data, 
                                             nComps, 'SLA_C')
intercepts = jackknife[1,,,]
coefs = jackknife[-1,,,]
pred_jackknife = matrix(rep(intercepts, length(val.plsr.data[,'SLA_C'])), byrow = T,
...
```{r SLA_C cal scatter, echo=FALSE}
...
cal_scatter_plot <- ggplot(cal.plsr.output, aes(x=Observed, y=Predicted)) + 
  ...
  labs(x=expression("\n Observed: SLA_C ("*cm^2*mg^-1*"C)"),
       y=expression("Predicted: SLA_C ("*cm^2*mg^-1*"C)")) +
...
```{r SLA_C cal residuals plot, echo=FALSE}
...
```{r SLA_C val CI plot, echo=FALSE}
val_CI_plot <- ggplot(val.plsr.output, aes(x=Observed, y=Predicted)) +
  geom_abline(intercept = 0, slope = 1, linewidth=0.3) + 
  geom_point(aes(col=Subpop, shape = Treatment)) + 
  scale_shape_manual(values = treatment_shape)+
  geom_errorbar(aes(ymin = lci, ymax = uci,col=Subpop), width=1, size = 0.1) +
  scale_color_manual(values = subpop_col)+
  #geom_smooth(method = "lm", formula = y ~ x, se = F) +
  scale_x_continuous(breaks = seq(round(rng[1]), round(rng[2]), length = 5), 
                     labels = seq(round(rng[1]), round(rng[2]), length = 5), 
...
```{r SLA_C val PI plot, echo=FALSE}
val_PI_plot <- ggplot(val.plsr.output, aes(x=Observed, y=Predicted)) +
  geom_abline(intercept = 0, slope = 1, linewidth=0.3) + 
  geom_point(aes(col=Subpop, shape = Treatment)) + 
  scale_shape_manual(values = treatment_shape)+
  geom_errorbar(aes(ymin = lpi, ymax = upi, col=Subpop), width=1, size = 0.1) +
  scale_color_manual(values = subpop_col)+
  #geom_smooth(method = "lm", formula = y ~ x, se = F) +
  scale_x_continuous(breaks = seq(round(rng[1]), round(rng[2]), length = 5), 
                     labels = seq(round(rng[1]), round(rng[2]), length = 5), 
...
```{r SLA_C val residuals plot, echo=FALSE}
...
```{r SLA_C save plots and data, eval=FALSE, echo = F}
ggsave(filename = paste0(file_path, "/SLA_C_cal_scatter_plot.pdf"), 
       plot = cal_scatter_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/SLA_C_cal_resid_plot.pdf"), 
       plot = cal_resid_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/SLA_C_val_CI_plot.pdf"), 
       plot = val_CI_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/SLA_C_val_PI_plot.pdf"), 
       plot = val_PI_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/SLA_C_val_resid_plot.pdf"), 
       plot = val_resid_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/SLA_C_selected_wv.pdf"), 
       plot = selected_wv, 
       device = "pdf", width = 7, height = 5, units = "cm", dpi = 400)

write.csv(x=reliefF, 
          file = paste0(file_path, "/SLA_C_reliefF_400.csv"), 
          row.names = F)
write.csv(x=cal.plsr.output, 
          file = paste0(file_path, "/SLA_C.cal.plsr.output.csv"), 
          row.names = F)
write.csv(x=val.plsr.output, 
          file = paste0(file_path, "/SLA_C.val.plsr.output.csv"), 
          row.names = F)
write.csv(x=out.jk.coefs, 
          file = paste0(file_path, "/SLA_C_out.jk.coefs.csv"), 
          row.names = F)
write.csv(x=coefs, 
          file = paste0(file_path, "/SLA_C_coefs.csv"), 
          row.names = T)
```

---
title: "Predict specific leaf area with respect to carbon from hyperspectral data"
date: "`r Sys.Date ()`"
output:
...
1. Used RReliefFexpRank algorithm to select wavelengths which had 300 top attributes values for PLSR modeling. These wavelengths were referred to as $W_{PLSR}$.    
2. Built a PLSR model with $W_{PLSR}$ as input and specific leaf area with respect to carbon (SLA_C_C, $cm^{2} mg ^{-1}C$) as output. Methods were adapted from Burnett et al. (2021).  
...
inVar <- "SLA_C"
file_path <- "./results/reliefF_plsr/SLA_C_RMSEP/"
```

```{r select SLA_C, echo = F, fig.align='center', fig.width = 4, fig.height = 4}
trait <- phyraw %>%
...
ref_trait_expRank <- attrEval(SLA_C ~., ref_trait_num, estimator = "RReliefFexpRank", ReliefIterations = nrow(trait)*50)
...
SLA_C_feature_plot
```
Figure 1: Attributes values of HSI data on Week 9 for SLA_C. Wavelengths with the top 300 attribute values are considered as important (red, $W_{PLSR}$) and are model input.  
```{r, echo=FALSE, eval=FALSE}
ggsave(filename = paste0(file_path, "SLA_C_expRank.png"), plot = ref_trait_expRank_plot, device="png", width = 30, height = 12, units = "cm", dpi = 400)
...
SLA_C_range <- range(trait$SLA_C)

cal_hist_plot <-ggplot(aes(x = SLA_C), data = cal.plsr.data) +
  geom_histogram(binwidth = 1, fill=I("#0072B2"), col=I("black")) +
  theme_bw(base_size = 18) +
  labs(x = expression("SLA_C ("*cm^2*mg^-1~C~")"), y = "Count") +
  ggtitle("Cal. (n = 70)") 

val_hist_plot <- ggplot(aes(x = SLA_C), data = val.plsr.data) +
  geom_histogram(binwidth = 1, fill=I("#0072B2"), col=I("black")) +
  theme_bw(base_size = 18) +
  labs(x = expression("SLA_C ("*cm^2*mg^-1~C~")"), y = "Count") +
  ggtitle("Val. (n = 18)")
...
cal_val_histograms 
```
Figure 2: Frequency distributions of SLA_C ($cm^{2}mg^{-1}C$) in the calibration (left, *n* = 70) and validation (right, *n* = 18) datasets. 
```{r save hist. of cal. and val., echo=FALSE, eval=FALSE, fig.align='center', fig.width = 5, fig.height = 5}
cal_hist_plot
ggsave(filename = paste0(file_path, "cal_SLA_C_histogram.png"), plot = cal_hist_plot, device="png", width = 12, height = 12, units = "cm", dpi = 300)
val_hist_plot
ggsave(filename = paste0(file_path, "val_SLA_C_histogram.png"), plot = val_hist_plot, device="png", width = 12, height = 12, units = "cm", dpi = 300)
write.csv(cal.plsr.data, file = paste0(file_path, "SLA_C_Cal_PLSR_Dataset.csv"), row.names=FALSE)
write.csv(val.plsr.data, file = paste0(file_path, "SLA_C_Val_PLSR_Dataset.csv"), row.names=FALSE)
```

2. Fitted a plsr model    
 
```{r determine number of components, echo = F, fig.align='center', fig.width = 7, fig.height = 5}
plsr.ini <- plsr(SLA_C~Spectra, scale=FALSE,
...
#png(filename = paste0(file_path, "SLA_C_minRMSEP_nComps.png"), bg = "white", width = 7, height = 5, units = "in", res = 400)
...
#dev.off()
```
Figure 3: Root mean square error of prediction (RMSEP) as a function of model component number in PLSR models. The vertical long-dashed blue line is the number of components used (eight). RMSEP is derived from leave-one-out cross validation and error bars are the standard error of the cross-validation residuals.

3. Calculated model performance metrics   
```{r model metrics, echo = F}
plsr.out <- plsr(SLA_C~Spectra, scale=FALSE, ncomp = nComps,
                 validation="LOO",trace=FALSE,data=cal.plsr.data, jackknife=TRUE)
fit <- plsr.out$fitted.values[,,nComps]
#calibration
cal.plsr.output <- data.frame(SLA_C = cal.plsr.data$SLA_C,
                              Treatment = cal.plsr.data$Treatment,
                              PLSR_Predicted=fit,
                              PLSR_CV_Predicted=as.vector(plsr.out$validation$pred[,,nComps]))
cal.plsr.output <- cal.plsr.output %>%
  mutate(PLSR_CV_Residuals = PLSR_CV_Predicted-SLA_C)
...
cal_SLA_C_range <- (max(cal.plsr.output$SLA_C)-min(cal.plsr.output$SLA_C))
cal.NRMSEP <-  round((sqrt(mean(cal.plsr.output$PLSR_CV_Residuals^2))/cal_SLA_C_range)*100,3)

val.plsr.output <- data.frame(SLA_C = val.plsr.data$SLA_C,
...
val.plsr.output <- val.plsr.output %>%
  mutate(PLSR_Residuals = PLSR_Predicted-SLA_C)
...
val_SLA_C_range <- (max(val.plsr.output$SLA_C)-min(val.plsr.output$SLA_C))
val.NRMSEP <- round((sqrt(mean(val.plsr.output$PLSR_Residuals^2))/val_SLA_C_range)*100,3)
```
...
```{r measured vs CV predicted in calibration dataset, echo=FALSE, fig.align='center', fig.height=12, fig.width=12}
...
rng_cals <- c(min(cal.plsr.output$SLA_C), max(cal.plsr.output$SLA_C))
...
plot(cal.plsr.output.N1$PLSR_CV_Predicted, cal.plsr.output.N1[,"SLA_C"],  
       lwd=1.6, xlim=c(rng_cals[1], rng_cals[2]), ylim=c(rng_cals[1], rng_cals[2]), 
       pch=21, col="black", bg = tCol[1],cex=2, 
       xlab=expression(bold("CV Predicted : SLA_C ("*cm^2*mg^-1~C~")")),
       ylab=expression(bold("Measured : SLA_C ("*cm^2*mg^-1~C~")")),
       cex.axis=3, cex.lab = 3, font.lab = 3, font = 2)
points(cal.plsr.output.N2$PLSR_CV_Predicted, cal.plsr.output.N2[,"SLA_C"], 
       pch=21, col="black", bg = tCol[2], cex=2)
abline(0,1,lty=2,lw=2)
legend(rng_cals[1], rng_cals[2], legend=expr.cal, bty="n", cex=2, y.intersp = 1.5)
box(lwd=2.2)
...
#dev.copy(png, file.path(paste0(file_path, "SLA_C_Cal_model.png")), height=2800, width=3200, res=340)
#dev.off()

```
Figure 4: Measured values versus cross-validation values in calibration dataset (*n* = 70) of SLA_C ($cm^{2} mg^{-1}C$).
...
plot(cal.plsr.output.N1$PLSR_CV_Predicted, cal.plsr.output.N1$PLSR_CV_Residuals, 
       lwd=1.6, xlim=c(rng_cals[1], rng_cals[2]), 
       pch=21, col="black", bg = tCol[1], cex=2, 
       xlab=expression(bold("CV Predicted : SLA_C ("*cm^2*mg^-1~C~")")),
...
box(lwd=2.2)
#dev.copy(png, file.path(paste0(file_path, "SLA_C_cal_resi.png")), height=2800, width=3200, res=340)
#dev.off()

```
Figure 5: Residuals from the cross validation values versus cross validation predicted values (*n* = 70) of SLA_C ($cm^{2} mg^{-1}C$).  
```{r measured vs pred with 95CI in validation dataset, echo=FALSE, fig.align='center', fig.height=12, fig.width=12}
...
par(mfrow=c(1,1), mar=c(6, 11, 1, 0.4), oma=c(0, 0.1, 0, 0.2), mgp = c(5, 2, 0)) 
plotrix::plotCI(val.plsr.output.N1$PLSR_Predicted, val.plsr.output.N1[,"SLA_C"],  
       ...
       xlab=expression(bold("Predicted : SLA_C ("*cm^2*mg^-1~C~")")),
       ylab=expression(bold("Measured : SLA_C ("*cm^2*mg^-1~C~")")),
       cex.axis=3, cex.lab =3, font.lab = 2, font = 2)
plotrix::plotCI(val.plsr.output.N2$PLSR_Predicted, val.plsr.output.N2[,"SLA_C"],  add = T,
...
abline(0,1,lty=2,lw=2)
legend(rng_cals[1], rng_cals[2], legend=expr, bty="n", cex=2, y.intersp = 1.5)
legend("bottomright", inset = c(0.1,0.1), legend = c("N1", "N2"), pch=21 , pt.bg = tCol,
       col = "black", cex=2, pt.cex = 2, text.col = "black", y.intersp = 1.5, bty = "n")
box(lwd=2.2)
#dev.copy(png, file.path(paste0(file_path, "SLA_C_val_model_with_95CI.png")), height=2800, width=3200, res=340)
#dev.off()
```
Figure 6: Measured values versus prediction in validation dataset (*n* = 18) of SLA_C ($cm^{2} mg^{-1}C$). Error bars represent 95%CIs
```{r residuals vs predicted in validation dataset, echo=FALSE, fig.align='center', fig.height=12, fig.width=12}
par(mfrow=c(1,1), mar=c(6, 11, 1, 1), oma=c(0, 0.1, 0, 0.2), mgp = c(5, 2, 0))
...
abline(h=0,lty=2,lw=2)
box(lwd=2.2)
#dev.copy(png, file.path(paste0(file_path, "SLA_C_val_resid_vs_pred.png")), height=2800, width=3200, res=340)
#dev.off()
```
Figure 7: Residuals from the external validation values versus predicted values (*n* = 18) of SLA_C ($cm^{2} mg^{-1}C$) in validation dataset.
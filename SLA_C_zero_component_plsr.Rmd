...
Predict specific leaf area with respect to carbon (SLA_C, $cm^2 C(mg)^{-1}$, *n* = 88, N1=N2=44) with hyperspectral data.  

# Methods  

1. Use RReliefFexpRank algorithm to select the top 300 wavelengths that were important for SLA_C.  
...
inVar <- "SLA_C"
file_path <- "./results/relieF_pca_plsr/SLA_C/"
```
...
wv_trait_expRank <- attrEval(SLA_C ~., wv_trait_2, estimator = "RReliefFexpRank", ReliefIterations = nrow(trait)*50) 
...
```
Figure 1: Attribute values of HSI data on Week 9 for SLA_C ($cm^2 C(mg)^{-1}$). Wavelengths with the top 300 attribute values are considered as important (red).  
```{r, include = FALSE}
...
HSI_p
#ggsave(filename = paste0(file_path, "/SLA_C_sel_300wv.png"), plot = HSI_p, device="png", width = 30, height = 12, units = "cm", dpi = 300)
```
...
sum(HSI_var_percent[1:3]) #96.2
```

```{r, echo = F, warnings = F, fig.align='center', fig.width = 4, fig.height = 4}
#png(filename = paste0(file_path, "/HSI_SLA_C_var_percent.png"), bg = "white", width = 10, height = 10, units = "in", res = 300)
...
#dev.off()
```
Figure 2: PCA scree plot. The first three PCs account for 96.2% of the variance. The top ten loadings in PC1 are wavelengths from 936 to 947 nm with one wavelength at 992 nm. The top ten loadings in PC2 and PC3 are wavelengths from 725 to 737 nm and from 2273 to 2491 nm, respectively. 
```{r, include=FALSE}
...
HSI_top_loadings <- data.frame(Trait = "SLA_C", PC = rep(c("PC1", "PC2", "PC3"), each = 10), 
...
#write.csv(file = paste0(file_path, "/HSI_SLA_C_top_loadings.csv"), x= HSI_top_loadings_2, row.names = F)
```
## Step 3: plsr  
```{r, include=FALSE}
`%notin%` <- Negate(`%in%`)
...
cal_hist_plot <- qplot(cal.plsr.data$SLA_C, geom="histogram", bins =5,
                       main = "Cal. (n = 70)",
                       xlab = expression("SLA_C ("~cm^2*C(mg)^-1~")"), ylab = "Count", 
                       fill=I("#0072B2"), col=I("black")) + theme_bw(base_size = 18)
 
val_hist_plot <- qplot(val.plsr.data$SLA_C, geom="histogram", bins =5,
                       main = "Val. (n = 18)",
                       xlab = expression("SLA_C ("~cm^2*C(mg)^-1~")"), ylab = "Count",
                       fill=I("#0072B2"),col=I("black")) + theme_bw(base_size = 18)
#val_hist_plot
cal_val_histograms <- grid.arrange(cal_hist_plot, val_hist_plot, ncol=2)
#ggsave(filename = paste0(file_path, "/cal_val_SLA_C_histograms.png"), plot = cal_val_histograms, device="png", width = 30, height = 12, units = "cm", dpi = 300)
...
```
Figure 3: Frequency distributions of SLA_C ($cm^2 C(mg)^{-1}$) in the calibration (left, *n* = 70) and evaluation (right, *n* = 18) datasets. 

2. Fit a plsr model    
Model: SLA_C = Scores of PCs. The scores of the PCs were not scaled. 
The model was cross-validated with leave-one-out method.  
```{r, echo = F, fig.align='center', fig.width = 7, fig.height = 5}
plsr.ini <- plsr(SLA_C~inputs_scores, scale=FALSE,
                 validation="LOO",trace=FALSE,data=cal.plsr.data)
#png(filename = paste0(file_path, "/SLA_C_nComps.png"), bg = "white", width = 10, height = 10, units = "in", res = 300)
par(mar = c(6, 7, 4, 2), mgp = c(5, 2, 0))
nComp <- selectNcomp(plsr.ini, method = "onesigma", plot = T, 
                     cex.axis = 2, cex.lab = 2, xlim = c(0, 15), ylim = c(0.6, 1.6), font.lab = 2, font = 2)
#dev.off()
```

...
Predict specific leaf area (SLA, $cm^{2} g^{-1}$) with hyperspectral imaging data (HSI data).  

# Methods  

1. Use RReliefFexpRank algorithm to select the top 300 wavelengths that were important for SLA.  
...
inVar <- "SLA"
file_path <- "./results/relieF_pca_plsr/SLA"
```

```{r, echo=FALSE, fig.align='center', fig.width = 4, fig.height = 4}
trait <- data.frame(ID = phyraw$ID,
                    SLA = phyraw$SLA, 
                    Treatment = phyraw$Treatment) %>% na.omit()
```
...
ref_trait_expRank <- attrEval(SLA ~., ref_trait_2, estimator = "RReliefFexpRank", ReliefIterations = nrow(trait)*50) 
...
```
Figure 1: Attribute values of HSI data on Week 9 for SLA ($cm^{2}g^{-1}$). Wavelengths with the top 300 attribute values are considered as important (red). 
```{r, echo=FALSE, eval=FALSE}
ggsave(filename = paste0(file_path, "/SLA_expRank_no_horizontal_line.png"), plot = ref_trait_expRank_plot, device="png", width = 30, height = 12, units = "cm", dpi = 300)
...
sum(ref_var_percent[1:3])
```

```{r, echo = F, warnings = F, fig.align='center', fig.width = 4, fig.height = 4}
#png(filename = "./results/plsr/SLA/ref_SLA_var_percent.png", bg = "white", width = 10, height = 10, units = "in", res = 300)
...
#dev.off()
```
Figure 2: Scree plot of the PCA. PCs 1-3 account for 95.5% of the variance. The top ten loadings in PC1, PC2 and PC3 are in the regions of 992 to 1077 nm, 706 to 728 nm and 2257 to 2491 nm, respectively.   
...
ref_PC3_loadings <- ref_loadings[order((ref_loadings[ ,3])^2, decreasing = T), 3]
ref_top10_loadings <- data.frame(Trait = "SLA", PC = rep(c("PC1", "PC2", "PC3"), each = 10), 
                    ID = c(as.numeric(gsub("_", ".", sub("...", "", names(ref_PC1_loadings[1:10])))), 
                           as.numeric(gsub("_", ".", sub("...", "", names(ref_PC2_loadings[1:10])))),
                           as.numeric(gsub("_", ".", sub("...", "", names(ref_PC3_loadings[1:10]))))
                           ),
                    loadings = c(as.numeric(ref_PC1_loadings[1:10]),                   
                                 as.numeric(ref_PC2_loadings[1:10]), 
                                 as.numeric(ref_PC3_loadings[1:10])))
ref_top10_loadings_2 <- ref_top10_loadings[!duplicated(ref_top10_loadings[ ,c('ID')]), ]
write.csv(file = "./results/plsr/SLA/ref_SLA_top10_loadings_2.csv", x= ref_top10_loadings_2, row.names = F)
```
...
The data were first grouped by treatments. Then, 80% of the data in each treatment were sampled for the calibrate dataset.
...
cal_hist_plot <- qplot(cal.plsr.data$SLA, geom="histogram", binwidth =26.3,
                       main = "Cal. (n = 74)",
                       xlab = expression("SLA ("*cm^2*g^-1~")"), ylab = "Count", 
                       fill=I("#0072B2"), col=I("black")) + 
                       theme_bw(base_size = 18)
val_hist_plot <- qplot(val.plsr.data$SLA, geom="histogram", binwidth =26.3,
                       main = "Val. (n = 20)",
                       xlab = expression("SLA ("*cm^2*g^-1~")"), ylab = "Count", 
                       fill=I("#0072B2"),col=I("black")) + 
                       theme_bw(base_size = 18)
...
```
Figure 3: Frequency distributions of SLA ($cm^{2}g^{-1}$) in the calibration (left, *n* = 74) and evaluation (right, *n* = 20) datasets. 
...
ggsave(filename = paste0(file_path, "/cal_val_SLA__histograms.png"), plot = cal_val_histograms, device="png", width = 30, height = 12, units = "cm", dpi = 300)
...
2. Fit a plsr model  
Model: SLA = Scores of PCs. The scores of the PCs were not scaled.  
The model was cross-validated with leave-one-out method.   

```{r, echo = F, fig.align='center', fig.width = 7, fig.height = 5}
plsr.ini <- plsr(SLA~inputs_scores, scale=FALSE,
                 validation="LOO",trace=FALSE,data=cal.plsr.data)
#png(filename = paste0(file_path, "/SLA_nComps.png"), bg = "white", width = 5, height = 5, units = "in", res = 400)
par(mar = c(6, 7, 4, 2), mgp = c(5, 2, 0))
nComp <- selectNcomp(plsr.ini, method = "onesigma", plot = T,
                     cex.axis = 2, cex.lab = 2, xlim = c(0, 15), ylim = c(18, 34),
                     font.lab = 2, font = 2)
#dev.off()
```
...
```{r, echo = F, fig.align='center', fig.width = 9, fig.height = 5}
plsr.out <- plsr(SLA~inputs_scores, scale=FALSE, ncomp = nComp,
                 validation="LOO",trace=FALSE,data=cal.plsr.data)
fit <- plsr.out$fitted.values[,,nComp]

cal.plsr.output <- data.frame(measured = cal.plsr.data$SLA,
                              PLSR_Predicted=fit,
...
val.plsr.output <- data.frame(measured = val.plsr.data$SLA,
...
text_xloc <- seq(rng[1], rng[2], length = 10)[4]
text_xloc_n <- seq(rng[1], rng[2], length = 10)[8]
text_R2 <- (rng[2]-rng[1])/10*9.5+rng[1]
text_RMSEP <- (rng[2]-rng[1])/10*8.5+rng[1]
text_NRMSEP <- (rng[2]-rng[1])/10*7.5+rng[1]
text_y_n <- seq(rng[1], rng[2], length = 10)[2]
...
cal_scatter_plot <- ggplot(cal.plsr.output, aes(x=measured, y=PLSR_CV_Predicted)) + 
  ...
  xlim(rng[1], rng[2]) + 
  ylim(rng[1], rng[2]) +
  labs(x=expression(bold("Measured: SLA ("*cm^2*g^-1~")")),
       y=expression(bold("Predicted: SLA ("*cm^2*g^-1~")"))) +
  theme(axis.text=element_text(size=25, face = "bold"), legend.position="none",
        axis.title=element_text(size=25, face = "bold"), 
        axis.title.x = element_text(margin = margin(t = 20, r = 0, b = 0, l = 0)),
        axis.text.x = element_text(angle = 0, vjust = 0.5, hjust = 1),
        axis.text.y = element_text(angle = 0, vjust = 1)) +
  annotate("text", size = 8, x= text_xloc, y = c(text_R2, text_RMSEP, text_NRMSEP), parse = TRUE, 
           label = c(paste("R^{2} == ", round(cal.R2, 2)), paste("RMSEP ==", round(cal.RMSEP, 2)), 
                     paste("NRMSEP ==", round(cal.NRMSEP, 2))))+
  annotate("text", x = text_xloc_n, y = text_y_n, parse = T, label = "Cal. (italic(n) == 74)", size = 8)
```
...
cal_resid_histogram <- ggplot(cal.plsr.output, aes(x=PLSR_CV_Residuals)) +
  geom_histogram(alpha=.5, position="identity", bins = 10, col="black") + 
...
cal_resid_histogram
ggsave(filename = paste0(file_path, "/SLA_Cal_Residualsplot.png"), plot = cal_resid_histogram, device="png", width = 12, height = 12, units = "cm", dpi = 300)
```
...
val_scatter_plot <- ggplot(val.plsr.output, aes(x=measured, y=PLSR_Predicted)) + 
  ...
  theme(axis.text=element_text(size=25, face = "bold"), legend.position="none",
        ...
        axis.text.y = element_text(angle = 0, vjust = 1))+
  annotate("text", size = 8, x= text_xloc, y = c(text_R2, text_RMSEP, text_NRMSEP), parse = TRUE, 
           label = c(paste("R^{2} == ", round(val.R2, 2)), paste("RMSEP ==", round(val.RMSEP, 2)), 
                     paste("NRMSEP ==", round(val.NRMSEP, 2))))+
  annotate("text", x = text_xloc_n, y = text_y_n, parse = T, label = "Val. (italic(n) == 20)", size = 8)
```

```{r, echo=FALSE, eval = F, fig.width=5, fig.height=5, fig.align='center'}  
val_scatter_plot
ggsave(filename = paste0(file_path, "/SLA_Val_lm_Scatterplot.png"), plot = val_scatter_plot, device="png", width = 15, height = 15, units = "cm", dpi = 400)
```
...
```{r, echo = F, eval = F, fig.align='center', fig.width = 5, fig.height = 5}
val_resid_histogram
ggsave(filename = paste0(file_path, "/SLA_Val_Residualsplot.png"), plot = val_resid_histogram, device="png", width = 12, height = 12, units = "cm", dpi = 300)
```

```{r,  echo = F, fig.align='center', fig.width = 12, fig.height = 6}
scatterplots <- grid.arrange(cal_scatter_plot, val_scatter_plot, ncol = 2)
```
Figure 5: Cross-validated values versus measured values (left, *n* = 70) and external validation values versus measured values (right, *n* = 18) of SLA ($cm^{2}g^{-1}$)
```{r, echo = F, fig.align='center', fig.width = 8, fig.height = 4}
histplots <-  grid.arrange(cal_resid_histogram, val_resid_histogram, ncol=2)
```
Figure 6: Residuals from the cross-validated values versus measured values (left, *n* = 74) and residuals from the external validation values versus measured values (right, *n* = 20) of SLA ($cm^{2}g^{-1}$)
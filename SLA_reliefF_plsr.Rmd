---
title: "Predict leaf-level specific leaf area from hyperspectral data"
date: "`r Sys.Date ()`"
output:
...
2. Built a PLSR model with $W_{PLSR}$ as input and leaf-level specific leaf area (SLA, $cm^{2} g^{-1}$) as output. Methods were adapted from Burnett et al. (2021).  
...
inVar <- "SLA"
file_path <- "./results/reliefF_plsr/SLA_minRMSEP/"
```

```{r select SLA, echo = F, fig.align='center', fig.width = 4, fig.height = 4}
trait <- phyraw %>%
...
ref_trait_expRank <- attrEval(SLA ~., ref_trait_num, estimator = "RReliefFexpRank", ReliefIterations = nrow(trait)*50)
...
SLA_feature_plot
```
Figure 1: Attributes values of HSI data on Week 9 for SLA. Wavelengths with the top 300 attribute values are considered as important (red, $W_{PLSR}$) and are model input.  
```{r, echo=FALSE, eval=FALSE}
ggsave(filename = paste0(file_path, "SLA_feature.png"), plot = SLA_feature_plot, device="png", width = 30, height = 12, units = "cm", dpi = 400)
...
SLA_range <- range(trait$SLA)
#sd(trait$CN)
cal_hist_plot <-ggplot(aes(x = SLA), data = cal.plsr.data) +
  geom_histogram(binwidth =20, fill=I("#0072B2"), col=I("black")) +
  coord_cartesian(xlim = c(40, 180)) +
  theme_bw(base_size = 18) +
  labs(x = expression("SLA ("*cm^2*g^-1~")"), y = "Count") +
  ggtitle("Cal. (n = 74)") 

val_hist_plot <- ggplot(aes(x = SLA), data = val.plsr.data) +
  geom_histogram(binwidth = 20, fill=I("#0072B2"), col=I("black")) +
  coord_cartesian(xlim = c(40, 180)) +
  theme_bw(base_size = 18) +
  labs(x = expression("SLA ("*cm^2*g^-1~")"), y = "Count") +
...
cal_val_histograms 
```
Figure 2: Frequency distributions of SLA ($cm^{2}g^{-1}$) in the calibration (left, *n* = 74) and validation (right, *n* = 20) datasets. 
```{r save hist. of cal. and val., echo=FALSE, eval=FALSE, fig.align='center', fig.width = 5, fig.height = 5}
cal_hist_plot
ggsave(filename = paste0(file_path, "cal_SLA_histogram.png"), plot = cal_hist_plot, device="png", width = 12, height = 12, units = "cm", dpi = 300)
val_hist_plot
ggsave(filename = paste0(file_path, "val_SLA_histogram.png"), plot = val_hist_plot, device="png", width = 12, height = 12, units = "cm", dpi = 300)
write.csv(cal.plsr.data, file = paste0(file_path, "SLA_Cal_PLSR_Dataset.csv"), row.names=FALSE)
write.csv(val.plsr.data, file = paste0(file_path, "SLA_Val_PLSR_Dataset.csv"), row.names=FALSE)
```

2. Fitted a plsr model    
 
```{r determine number of components, echo = F, fig.align='center', fig.width = 7, fig.height = 5}
plsr.ini <- plsr(SLA~Spectra, scale=FALSE,
...
#png(filename = paste0(file_path, "SLA_minRMSEP_nComps.png"), bg = "white", width = 7, height = 5, units = "in", res = 400)
...
#dev.off()
```
Figure 3: Root mean square error of prediction (RMSEP) as a function of model component number in PLSR models. The vertical long-dashed blue line is the number of components used (seven). RMSEP is derived from leave-one-out cross validation and error bars are the standard error of the cross-validation residuals.

3. Calculated model performance metrics   
```{r model metrics, echo = F}
plsr.out <- plsr(SLA~Spectra, scale=FALSE, ncomp = nComps,
                 validation="LOO",trace=FALSE,data=cal.plsr.data, jackknife=TRUE)
fit <- plsr.out$fitted.values[,,nComps]
#calibration
cal.plsr.output <- data.frame(SLA = cal.plsr.data$SLA,
                              Treatment = cal.plsr.data$Treatment,
                              PLSR_Predicted=fit,
                              PLSR_CV_Predicted=as.vector(plsr.out$validation$pred[,,nComps]))
cal.plsr.output <- cal.plsr.output %>%
  mutate(PLSR_CV_Residuals = PLSR_CV_Predicted-SLA)
...
cal_SLA_range <- (max(cal.plsr.output$SLA)-min(cal.plsr.output$SLA))
cal.NRMSEP <-  round((sqrt(mean(cal.plsr.output$PLSR_CV_Residuals^2))/cal_SLA_range)*100,3)

val.plsr.output <- data.frame(SLA = val.plsr.data$SLA,
...
val.plsr.output <- val.plsr.output %>%
  mutate(PLSR_Residuals = PLSR_Predicted-SLA)
...
val_SLA_range <- (max(val.plsr.output$SLA)-min(val.plsr.output$SLA))
val.NRMSEP <- round((sqrt(mean(val.plsr.output$PLSR_Residuals^2))/val_SLA_range)*100,3)
```
...
rng_cals <- c(min(cal.plsr.output$SLA), max(cal.plsr.output$SLA))
...
plot(cal.plsr.output.N1$PLSR_CV_Predicted, cal.plsr.output.N1[,"SLA"],  
       lwd=1.6, xlim=c(rng_cals[1], rng_cals[2]), ylim=c(rng_cals[1], rng_cals[2]), 
       pch=21, col="black", bg = tCol[1],cex=2, 
       xlab=expression(bold("CV Predicted : SLA ("*cm^2*g^-1~")")),
       ylab=expression(bold("Measured : SLA ("*cm^2*g^-1~")")),
       cex.axis=3, cex.lab = 3, font.lab = 3, font = 2)
points(cal.plsr.output.N2$PLSR_CV_Predicted, cal.plsr.output.N2[,"SLA"], 
...
#dev.copy(png, file.path(paste0(file_path, "SLA_Cal_model.png")), height=2800, width=3200, res=340)
#dev.off()

```
Figure 4: Measured values versus cross-validation values in calibration dataset (*n* = 74) of SLA ($cm^{2} g^{-1}$).
...
box(lwd=2.2)
#dev.copy(png, file.path(paste0(file_path, "SLA_cal_resi.png")), height=2800, width=3200, res=340)
#dev.off()

```
Figure 5: Residuals from the cross validation values versus cross validation predicted values (*n* = 74) of SLA ($cm^{2} g^{-1}$).  
...
par(mfrow=c(1,1), mar=c(6, 11, 1, 0.4), oma=c(0, 0.1, 0, 0.2), mgp = c(5, 2, 0)) 
plotrix::plotCI(val.plsr.output.N1$PLSR_Predicted, val.plsr.output.N1[,"SLA"],  
       ...
       xlab=expression(bold("Predicted : SLA ("*cm^2*g^-1~")")),
       ylab=expression(bold("Measured : SLA ("*cm^2*g^-1~")")),
       cex.axis=3, cex.lab =3, font.lab = 2, font = 2)
plotrix::plotCI(val.plsr.output.N2$PLSR_Predicted, val.plsr.output.N2[,"SLA"],  add = T,
...
legend("bottomright", inset = c(0.1,0.1), legend = c("N1", "N2"), pch=21 , pt.bg = tCol,
       col = "black", cex=2, pt.cex = 2, text.col = "black", y.intersp = 1.5, bty = "n")
box(lwd=2.2)
#dev.copy(png, file.path(paste0(file_path, "SLA_val_model_with_95CI.png")), height=2800, width=3200, res=340)
#dev.off()
```
Figure 6: Measured values versus prediction in validation dataset (*n* = 20) of SLA ($cm^{2} g^{-1}$). Error bars represent 95%CIs
...
abline(h=0,lty=2,lw=2)
box(lwd=2.2)
#dev.copy(png, file.path(paste0(file_path, "SLA_val_resid_vs_pred.png")), height=2800, width=3200, res=340)
#dev.off()
```
Figure 7: Residuals from the external validation values versus predicted values (*n* = 20) of SLA ($cm^{2} g^{-1}$) in validation dataset.
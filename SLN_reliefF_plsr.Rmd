---
title: "Predict specific leaf nitrogen with hyperspectral data.  "
date: "`r Sys.Date ()`"
output: pdf_document
---
...
2. Built a PLSR model with $W_{PLSR}$ as input and specific leaf nitrogen (SLN, $N (mg) cm^{-2}$, *n* = 88) as output. Methods were adapted from Burnett et al. (2021).  
...
ref_trait_expRank <- attrEval(SLN ~., ref_trait_num, estimator = "RReliefFexpRank", ReliefIterations = nrow(trait)*50) 
...
SLN_feature_plot
```
Figure 1: Attribute values of HSI data on Week 9 for SLN. Wavelengths with the top 300 attribute values are considered as important (red, $W_{PLSR}$) and are used for model input.  
...
```{r cal. and val. dataset, echo = F, fig.align='center', fig.width = 9, fig.height = 5}
...
cal_hist_plot <- qplot(cal.plsr.data$SLN, geom="histogram", bins =10,
                       main = "Cal. (n = 70)",
                       xlab = expression("SLN (mg N"~cm^-2~")"), ylab = "Count", 
...
```
Figure 2: Frequency distributions of SLN in the calibration (left, *n* = 70) and validation (right, *n* = 18) datasets. 
  
2. Fitted a plsr model    
 
```{r determine number of components, echo = F, fig.align='center', fig.width = 7, fig.height = 5}
plsr.ini <- plsr(SLN~Spectra, scale=FALSE,
...
cal_SLN_range <- (max(cal.plsr.output$SLN)-min(cal.plsr.output$SLN))
cal.NRMSEP <-  round((sqrt(mean(cal.plsr.output$PLSR_CV_Residuals^2))/cal_SLN_range)*100,3)

val.plsr.output <- data.frame(SLN = val.plsr.data$SLN,
...
val.plsr.output <- val.plsr.output %>%
  mutate(PLSR_Residuals = PLSR_Predicted-SLN)
...
val_SLN_range <- (max(val.plsr.output$SLN)-min(val.plsr.output$SLN))
val.NRMSEP <- round((sqrt(mean(val.plsr.output$PLSR_Residuals^2))/val_SLN_range)*100,3)
```
...
plot(cal.plsr.output.N1$PLSR_CV_Predicted, cal.plsr.output.N1[,"SLN"], 
       lwd=1.6, xlim=c(rng_vals[1], rng_vals[2]), ylim=c(rng_vals[1], rng_vals[2]), 
       pch=21, col="black", bg = tCol[1], cex=2, 
       xlab=expression("CV Predicted SLN (mg N"~cm^-2~")"),
       ylab=expression("Measured SLN (mg N"~cm^-2~")"),
       cex.axis=3, cex.lab = 3, font.lab = 2, font = 2)
points(cal.plsr.output.N2$PLSR_CV_Predicted, cal.plsr.output.N2[,"SLN"], 
...
legend("bottomright", inset = c(0.1,0.1), legend = c("N1", "N2"), pch=21 , pt.bg = tCol,
       col = "black", cex=2, pt.cex = 2, text.col = "black", y.intersp = 1.5)
box(lwd=2.2)
#dev.copy(png, file.path(paste0(file_path, "N_Cal_model.png")), height=2800, width=3200, res=340)
#dev.off()
```
Figure 4: Measured values  versus cross-validation values in calibration dataset (*n* = 70) of SLN ($N (mg) cm^{-2}$).   
...
```
Figure 5: Residuals from the cross validation values versus cross validation predicted values (*n* = 70) of SLN ($N (mg) cm^{-2}$).  
...
rng_vals <- c(min(val.plsr.output$LCI), max(val.plsr.output$UCI))
par(mfrow=c(1,1), mar=c(6, 7, 1, 0.4), oma=c(0, 0.1, 0, 0.2), mgp = c(5, 2, 0)) 
plotrix::plotCI(val.plsr.output.N1$PLSR_Predicted, val.plsr.output.N1[,"SLN"], 
       ...
       xlab=expression("Predicted SLN (mg N"~cm^-2~")"),
       ylab=expression("Measured SLN (mg N"~cm^-2~")"),
       cex.axis=3, cex.lab = 3, font.lab = 2, font = 2)
plotrix::plotCI(val.plsr.output.N2$PLSR_Predicted, val.plsr.output.N2[,"SLN"], add = T,
...
#dev.off()
```
Figure 6: Measured values versus prediction in validation dataset (*n* = 18) of SLN ($N (mg) cm^{-2}$). Error bars represent 95%CIs.  
...
```
Figure 7: Residuals from the external validation values versus predicted values (*n* = 18) of SLN ($N (mg) cm^{-2}$).  

---
title: "Use side-view HSI data to predict Vcmax on W13"
date: "`r Sys.Date ()`"
output:
...
file_path <- "./results/reliefF_plsr/sideviewA/Vcmax_W13_n_500_wv"
```
...
```{r data, echo=FALSE}
y_metadata <- phyraw %>%
  filter(Jday_start == 265) %>%
  select("ID", "Vcmax", "Treatment", "Subpop") %>%
  mutate(ID = factor(ID)) %>%
  na.omit() 
...
pre_cal = input_for_reliefF(X_IDdata = X_IDdata, y_metadata = y_metadata, 
                            cal.ID = cal.ID, y_name = 'Vcmax', 
                            num_X_names = num_X_names)

reliefF = reliefF_algorithm(pre_cal = pre_cal, 
                                y_name = 'Vcmax', num_X_names = num_X_names) 
reliefF_star =  reliefF[order(reliefF$value, decreasing = T)[1:500], ] 
tail(reliefF_star)
```
...
cal_ref_95perc = ggplot()+
  ...
  labs(title = "Vcmax calibration dataset", x= "Wavelength (nm)", y = "Reflectance (%)") +
...
val_ref_95perc = ggplot()+
  ...
  scale_y_continuous(breaks = seq(sel_wv_ref_min, sel_wv_ref_max, length.out = 3), 
                     labels = round(seq(sel_wv_ref_min, sel_wv_ref_max, length.out = 3), 3),
                     limits = c(sel_wv_ref_min, sel_wv_ref_max)) +
  labs(title = "Vcmax validation dataset", x= "Wavelength (nm)", y = "Reflectance (%)") +
  theme_bw(base_size=18) +
  theme(
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
...
selected_wv = ggplot()+
  geom_vline(aes(xintercept=variable), data=cal.plsr.long, color = "orange") + 
...
maxComps <- 15
pdf(file = paste0(file_path, "/Vcmax_W13_nComps.pdf"), bg = "white", width = 7, height = 5)
...
coefs = jackknife[-1,,,]
pred_jackknife = matrix(rep(intercepts, length(val.plsr.data[,'Vcmax'])), byrow = T,
...
cal_scatter_plot <- ggplot(cal.plsr.output, aes(x=Observed, y=Predicted)) + 
  ...
  labs(x=expression("\n Observed:"~V[cmax]~"("~mu*mol~m^-2~s^-1~")"),
       y=expression("Predicted:"~V[cmax]~"("~mu*mol~m^-2~s^-1~")")) +
...
val_CI_plot <- ggplot(val.plsr.output, aes(x=Observed, y=Predicted)) +
  ...
  geom_point(aes(col=Subpop, shape = Treatment)) + 
  scale_shape_manual(values = treatment_shape)+
  geom_errorbar(aes(ymin = lci, ymax = uci,col=Subpop), width=5, size = 0.1) +
  scale_color_manual(values = subpop_col)+
  #geom_smooth(method = "lm", formula = y ~ x, se = F) +
  scale_x_continuous(breaks = seq(round(rng[1]), round(rng[2]), length = 5), 
                     labels = seq(round(rng[1]), round(rng[2]), length = 5), 
...
val_PI_plot <- ggplot(val.plsr.output, aes(x=Observed, y=Predicted)) +
  ...
  geom_point(aes(col=Subpop, shape = Treatment)) + 
  scale_shape_manual(values = treatment_shape)+
  geom_errorbar(aes(ymin = lpi, ymax = upi, col=Subpop), width=5, size = 0.1) +
  scale_color_manual(values = subpop_col)+
  #geom_smooth(method = "lm", formula = y ~ x, se = F) +
  scale_x_continuous(breaks = seq(round(rng[1]), round(rng[2]), length = 5), 
                     labels = seq(round(rng[1]), round(rng[2]), length = 5), 
...
ggsave(filename = paste0(file_path, "/Vcmax_W13_cal_scatter_plot.pdf"), 
       plot = cal_scatter_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/Vcmax_W13_cal_resid_plot.pdf"), 
       plot = cal_resid_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/Vcmax_W13_val_CI_plot.pdf"), 
       plot = val_CI_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/Vcmax_W13_val_PI_plot.pdf"), 
       plot = val_PI_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/Vcmax_W13_val_resid_plot.pdf"), 
       plot = val_resid_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/Vcmax_W13_selected_wv.pdf"), 
       plot = selected_wv, 
       device = "pdf", width = 7, height = 5, units = "cm", dpi = 400)

write.csv(x=reliefF, 
          file = paste0(file_path, "/Vcmax_W13_reliefF_500.csv"), 
          row.names = F)
write.csv(x=cal.plsr.output, 
          file = paste0(file_path, "/Vcmax_W13.cal.plsr.output.csv"), 
          row.names = F)
write.csv(x=val.plsr.output, 
          file = paste0(file_path, "/Vcmax_W13.val.plsr.output.csv"), 
          row.names = F)
write.csv(x=out.jk.coefs, 
          file = paste0(file_path, "/Vcmax_W13_out.jk.coefs.csv"), 
          row.names = F)
write.csv(x=coefs, 
          file = paste0(file_path, "/Vcmax_W13_coefs.csv"), 
          row.names = T)
```

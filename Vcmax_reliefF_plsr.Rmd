---
title: "Predict the maximum carboxylation rate limited by Rubisco (Vcmax) from hyperspectral data"
date: "`r Sys.Date ()`"
output: pdf_document
---
...
2. Built a PLSR model with $W_{PLSR}$ as input and the maximum carboxylation rate limited by Rubisco (Vcmax, $\mu molm^{-2}s^{-1}$, *n* = 89) as output. Methods were adapted from Burnett et al. (2021).  

```{r import data, include=FALSE}
phyraw <- read.csv('./data/ground_truth/phyraw_forHSI_2022.csv', header = T, sep = ',', dec = '.')
ref_W9_meta <- read.csv('./data/hyperspectral_imaging/HSI_side_for_biotraits_mod_W9.csv', header = T, sep = ',', dec = '.')
ref_W13_meta <- read.csv('./data/hyperspectral_imaging/HSI_side_for_biotraits_mod_W13.csv', header = T, sep = ',', dec = '.')

inVar <- "Vcmax"
file_path <- "./results/reliefF_plsr/Vcmax/"
```

```{r combine phy data from W6 and W13, echo = F, fig.align='center'}
Vcmax_df <- phyraw[complete.cases(phyraw$Vcmax), c("ID", "Jday_start", "Treatment", "Accession", "Subpop", "Vcmax")] 
head(Vcmax_df)
Vcmax_W9 <- Vcmax_df %>%
  filter(Jday_start == "237") %>%
  mutate(Week = "9")
str(Vcmax_W9)
...
Vcmax_W13 <-  Vcmax_df %>%
  ...
  mutate(Week = "13")
str(Vcmax_W13)
Vcmax_bind <- rbind(Vcmax_W9, Vcmax_W13)
Vcmax_bind$Week <- factor(Vcmax_bind$Week, levels = c('9', "13"))

Vcmax_hist <- ggplot(aes(x=Vcmax, fill=Week), data = Vcmax_bind) +
    geom_histogram( color="#e9ecef", alpha=0.6, position = 'identity', bins = 25) +
    scale_fill_manual(values=c("#69b3a2", "#404080")) + 
    labs(x = "Vcmax") + theme_bw()
Vcmax_hist 
```
Figure 1. Vcmax ($\mu molm^{-2}s^{-1}$, *n* = 89) of each rice plants on weeks 9 (green) and 13 (purple).  
```{r combine HSI data between W6 and W13, echo = F, fig.align='center'}
ref_W9_meta <- ref_W9_meta[ref_W9_meta$ID %in% Vcmax_W9$ID, -c(746:762)] #drop columns with wv > 2500
ref_W9_week <-  ref_W9_meta %>%
  mutate(Week = "9",
         ID = paste(ref_W9_meta$ID, "_W9", sep=''))
ref_W9_long <- melt(ref_W9_week, id = c('ID', "Treatment", "Accession", "Subpop", "Country", "Week"))
ref_W9_long$variable <- as.numeric(gsub("_", ".", sub("...", "",  ref_W9_long$variable)))

ref_W9_line <- ggplot(aes(x = variable, y = value), data = ref_W9_long) +  
  geom_line(aes(group = ID), size = 0.5) +
  scale_x_continuous(breaks = seq(400, 2500, by = 300), labels = seq(400, 2500, by = 300))+
  labs( x = "Wavelength (nm)") + theme_bw() 

ref_W13_meta <- ref_W13_meta[ref_W13_meta$ID %in% Vcmax_W13$ID, -c(746:762)]
ref_W13_week <-  ref_W13_meta %>%
  mutate(Week = "13",
         ID = paste(ref_W13_meta$ID, "_W13", sep=''))
ref_W13_long <- melt(ref_W13_week, id = c('ID', "Treatment", "Accession", "Subpop", "Country", "Week"))
ref_W13_long$variable <- as.numeric(gsub("_", ".", sub("...", "",  ref_W13_long$variable)))

ref_W13_line <- ggplot(aes(x = variable, y = value), data = ref_W13_long) +  
  geom_line(aes(group = ID), size = 0.5) +
  scale_x_continuous(breaks = seq(400, 2500, by = 300), labels = seq(400, 2500, by = 300))+
  labs( x = "Wavelength (nm)") + theme_bw() 

ref_long <- rbind(ref_W9_long, ref_W13_long)
ref_long$Week <- factor(ref_long$Week, levels = c('9', "13"))
 
ref_line <- ggplot(aes(x = variable, y = value), data = ref_long) +  
  geom_line(aes(group = ID), size = 0.5) +
  scale_x_continuous(breaks = seq(400, 2500, by = 300), labels = seq(400, 2500, by = 300))+
  labs( x = "Wavelength (nm)") + theme_bw() + facet_wrap(~Week, nrow = 2)
ref_line
```
Figure 2. Reflectance on Week 9 (top, *n* = 46) and Week 13 (bottom, *n* = 43) of each plant.  

## Step 1: Feature selection    

```{r, echo=FALSE, fig.align='center', fig.width = 10, fig.height = 5}
ref_meta <- rbind(ref_W9_week, ref_W13_week)
ref <- ref_meta %>%
  select(-c(Treatment, Accession, Subpop, Country, Week))

Vcmax_W9$ID <- paste(Vcmax_W9$ID, "_W9", sep='')
Vcmax_W13$ID <- paste(Vcmax_W13$ID, "_W13", sep='')
trait_bind <- rbind(Vcmax_W9, Vcmax_W13)

ref_trait <- merge(ref, select(trait_bind, c(ID, Vcmax)), by = "ID")
ref_trait_num <- ref_trait[ ,-1] # drop ID

ref_trait_expRank <- attrEval(Vcmax ~., ref_trait_num, estimator = "RReliefFexpRank", ReliefIterations = nrow(trait_bind)*50) 
...
Vcmax_feature_plot
```
Figure 3: Attributes values of HSI data on Weeks 9 and 13 for Vcmax ($\mu molm^{-2}s^{-1}$, *n* = 89). Wavelengths with the top 300 attribute values are considered as important (red, $W_{PLSR}$) and are used for model input. 

```{r att anotate output, echo=FALSE, eval = F, fig.align='center', fig.width = 10, fig.height = 4}
Vcmax_feature_plot
ggsave(filename = paste0(file_path, "Vcmax_feature.png"), plot = E_feature_plot, device = "png", width = 30, height = 12, units = "cm", dpi = 400)
```
...
plsr_data <- data.frame(trait_bind, Spectra = I(ref_select))
set.seed(162425)
prop <- 0.8
...
cal_hist_plot <- qplot(cal.plsr.data$Vcmax, geom="histogram", bins =10,
                       main = "Cal. (n = 70)",
                       xlab = expression("Vcmax ("~mu*mol*m^-2*s^-1*")"), ylab = "Count", 
                       fill=I("#0072B2"), col=I("black")) + theme_bw(base_size = 18) +
  theme(axis.text.x = element_text(hjust = 1))

val_hist_plot <- qplot(val.plsr.data$Vcmax, geom="histogram", bins =7,
                       main = "Val. (n = 19)",
                       xlab = expression("Vcmax ("~mu*mol*m^-2*s^-1*")"), ylab = "Count", 
                       fill=I("#0072B2"), col=I("black")) + theme_bw(base_size = 18)
cal_val_histograms <- grid.arrange(cal_hist_plot, val_hist_plot, ncol=2)
```
Figure 4: Frequency distributions of Vcmax ($\mu molm^{-2}s^{-1}$) in the calibration (*n* = 70) and validation (*n* = 19) datasets.  

2. Fitted a plsr model    

```{r determine number of components, echo = F, fig.align='center', fig.width = 9, fig.height = 5}
plsr.ini <- plsr(Vcmax~Spectra, scale=FALSE,
...
cal_Vcmax_range <- (max(cal.plsr.output$Vcmax)-min(cal.plsr.output$Vcmax))
cal.NRMSEP <-  round((sqrt(mean(cal.plsr.output$PLSR_CV_Residuals^2))/cal_Vcmax_range)*100,3)

val.plsr.output <- data.frame(Vcmax = val.plsr.data$Vcmax,
...
val.plsr.output <- val.plsr.output %>%
  mutate(PLSR_Residuals = PLSR_Predicted-Vcmax)
...
val_Vcmax_range <- (max(val.plsr.output$Vcmax)-min(val.plsr.output$Vcmax))
val.NRMSEP <- round((sqrt(mean(val.plsr.output$PLSR_Residuals^2))/val_Vcmax_range)*100,3)
```
...
par(mfrow=c(1,1), mar=c(6, 11, 1, 0.4), oma=c(0, 0.1, 0, 0.2), mgp = c(5, 2, 0))
plot(cal.plsr.output.N1$PLSR_CV_Predicted, cal.plsr.output.N1[,"Vcmax"], 
       lwd=1.6, xlim=c(rng_vals[1], rng_vals[2]), ylim=c(rng_vals[1], rng_vals[2]), 
       pch=21, col="black", bg = tCol[1], cex=2, 
       xlab=expression("CV Predicted Vcmax ("~mu*mol*m^-2*s^-1*")"),
       ylab=expression("Measured Vcmax ("~mu*mol*m^-2*s^-1*")"),
       cex.axis=3, cex.lab = 3, font.lab = 2, font = 2)
points(cal.plsr.output.N2$PLSR_CV_Predicted, cal.plsr.output.N2[,"Vcmax"], 
...
#dev.off()
```
Figure 6: Measured values  versus cross-validation values in calibration dataset (*n* = 70) of Vcmax ($\mu molm^{-2}s^{-1}$) .   
...
```
Figure 7: Residuals from the cross validation values versus cross validation predicted values (*n* = 70) of Vcmax ($\mu molm^{-2}s^{-1}$)   
...
par(mfrow=c(1,1), mar=c(6, 11, 1, 0.4), oma=c(0, 0.1, 0, 0.2), mgp = c(5, 2, 0)) 
plotrix::plotCI(val.plsr.output.N1$PLSR_Predicted, val.plsr.output.N1[,"Vcmax"], 
       ...
       ylab=expression("Measured Vcmax ("~mu*mol*m^-2*s^-1*")"),
       cex.axis=3, cex.lab = 3, font.lab = 2, font = 2)
plotrix::plotCI(val.plsr.output.N2$PLSR_Predicted, val.plsr.output.N2[,"Vcmax"], add = T,
...
#dev.off()
```
Figure 8: Measured values versus prediction in validation dataset (*n* = 19) of Vcmax ($\mu molm^{-2}s^{-1}$)   . Error bars represent 95%CIs.  
...
```
Figure 9: Residuals from the external validation values versus predicted values (*n* = 19) of Vcmax ($\mu molm^{-2}s^{-1}$)     

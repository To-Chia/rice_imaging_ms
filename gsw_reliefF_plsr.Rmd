---
title: "Predict pre-dawn leaf-level stomatal conductance to water from hyperspectral data."
date: "`r Sys.Date ()`"
output: pdf_document
---
...
2. Built a PLSR model with $W_{PLSR}$ as input and *gsw* ($molm^{-2}s^{-1}$) as output. Methods were adapted from Burnett et al. (2021).  
...
inVar <- "gsw"
file_path <- "./results/reliefF_plsr/gsw/"
```

```{r select trait, echo = F, fig.align='center', fig.width = 4, fig.height = 4}
trait <- phyraw %>%
  select("ID", "gsw", "Treatment") %>%
  na.omit() 
...
ref_trait_expRank <- attrEval(gsw ~., ref_trait_num, estimator = "RReliefFexpRank", ReliefIterations = nrow(trait)*50) 
...
gsw_feature_plot
```
Figure 1: Attributes values of HSI data on Week 6 for pre-dawn leaf-level stomatal conductance to water (gsw, $molm^{-2}s^{-1}$, *n* = 94). Wavelengths with the top 300 attribute values are considered as important (red, $W_{PLSR}$) and are used for model input.
...
cal_hist_plot <- qplot(cal.plsr.data$gsw, geom="histogram", bins =10,
                       main = "Cal. (n = 74)",
                       xlab = expression("gsw ("*mol*m^-2*s^-1*")"), ylab = "Count", 
                       fill=I("#0072B2"), col=I("black")) + theme_bw(base_size = 18)
val_hist_plot <- qplot(val.plsr.data$gsw, geom="histogram", bins =5,
                       main = "Cal. (n = 20)",
                       xlab = expression("gsw ("*mol*m^-2*s^-1*")"), ylab = "Count",
                       fill=I("#0072B2"),col=I("black")) + theme_bw(base_size = 18)

cal_val_histograms <- grid.arrange(cal_hist_plot, val_hist_plot, ncol=2)
#ggsave(filename = paste0(file_path, "cal_val_gsw_histograms.png"), plot = cal_val_histograms, device="png", width = 30, height = 12, units = "cm", dpi = 300)
#write.csv(cal.plsr.data, file = paste0(file_path, "gsw_Cal_PLSR_Dataset.csv"), row.names=FALSE)
#write.csv(val.plsr.data, file = paste0(file_path, "gsw_Val_PLSR_Dataset.csv"), row.names=FALSE)

```
Figure 2: Frequency distributions of *gsw* ($molm^{-2}s^{-1}$) in the calibration (left, *n* = 74) and validation (right, *n* = 20) datasets. 
\newpage  

2. Fitted a plsr model    

```{r determine number of components, echo = F, fig.align='center', fig.width = 9, fig.height = 5}
plsr.ini <- plsr(gsw~Spectra, scale=FALSE,
...
plsr.out <- plsr(gsw~Spectra, scale=FALSE, ncomp = nComps,
                 validation="LOO",trace=FALSE,data=cal.plsr.data, jackknife=TRUE)
fit <- plsr.out$fitted.values[,,nComps]

cal.plsr.output <- data.frame(gsw = cal.plsr.data$gsw,
                              Treatment = cal.plsr.data$Treatment,
                              PLSR_Predicted=fit,
                              PLSR_CV_Predicted=as.vector(plsr.out$validation$pred[,,nComps]))
cal.plsr.output <- cal.plsr.output %>%
  mutate(PLSR_CV_Residuals = PLSR_CV_Predicted-gsw)
...
cal_gsw_range <- (max(cal.plsr.output$gsw)-min(cal.plsr.output$gsw))
cal.NRMSEP <-  round((sqrt(mean(cal.plsr.output$PLSR_CV_Residuals^2))/cal_gsw_range)*100,3)

val.plsr.output <- data.frame(gsw = val.plsr.data$gsw,
...
val.plsr.output <- val.plsr.output %>%
  mutate(PLSR_Residuals = PLSR_Predicted-gsw)
...
val_gsw_range <- (max(val.plsr.output$gsw)-min(val.plsr.output$gsw))
val.NRMSEP <- round((sqrt(mean(val.plsr.output$PLSR_Residuals^2))/val_gsw_range)*100,3)
```
...
plot(cal.plsr.output.N1$PLSR_CV_Predicted, cal.plsr.output.N1[,"gsw"], 
       lwd=1.6, xlim=c(rng_vals[1], rng_vals[2]), ylim=c(rng_vals[1], rng_vals[2]), 
       pch=21, col="black", bg = tCol[1], cex=2, 
       xlab=expression("CV Predicted gsw ("*mol*m^-2*s^-1*")"),
       ylab=expression("Measured gsw ("*mol*m^-2*s^-1*")"),
       cex.axis=3, cex.lab = 3, font.lab = 2, font = 2)
points(cal.plsr.output.N2$PLSR_CV_Predicted, cal.plsr.output.N2[,"gsw"], 
...
#dev.off()
```
Figure 4: Measured values versus cross-validation values in calibration dataset (*n* = 74) of gsw ($molm^{-2}s^{-1}$) .   
...
```
Figure 5: Residuals from the cross validation values versus cross validation predicted values (*n* = 74) of gsw ($molm^{-2}s^{-1}$)   
...
par(mfrow=c(1,1), mar=c(6, 11, 1, 0.4), oma=c(0, 0.1, 0, 0.2), mgp = c(5, 2, 0)) 
plotrix::plotCI(val.plsr.output.N1$PLSR_Predicted, val.plsr.output.N1[,"gsw"], 
       ...
       ylab=expression("Measured gsw ("*mol*m^-2*s^-1*")"),
       cex.axis=3, cex.lab = 3, font.lab = 2, font = 2)
plotrix::plotCI(val.plsr.output.N2$PLSR_Predicted, val.plsr.output.N2[,"gsw"], add = T,
...
#dev.off()
```
Figure 6: Measured values versus prediction in validation dataset (*n* = 20) of gsw ($molm^{-2}s^{-1}$). Error bars represent 95%CIs.  
...
```
Figure 7: Residuals from the external validation values versus predicted values (*n* = 20) of gsw ($molm^{-2}s^{-1}$)     

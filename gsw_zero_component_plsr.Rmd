...
Predict *gsw* ($molm^{-2}s^{-1}$, *n* = 94, N1 = N2 = 47) with hyperspectral data.  

# Methods  

1. Use RReliefFexpRank algorithm to select the top 300 wavelengths that were important for *gsw* ($molm^{-2}s^{-1}$).   
...
inVar <- "gsw"
file_path <- "./results/relieF_pca_plsr/gsw/"
```
...
wv_trait_expRank <- attrEval(gsw ~., wv_trait_2, estimator = "RReliefFexpRank", ReliefIterations = nrow(trait)*50) 
...
```
Figure 1: Attribute values of HSI data on Week 6 for *gsw*. Wavelengths with the top 300 attribute values are considered as important (red).  
```{r, include = FALSE}
...
HSI_p
#ggsave(filename = paste0(file_path, "gsw_sel_300wv.png"), plot = HSI_p, device="png", width = 30, height = 12, units = "cm", dpi = 300)
...
sum(HSI_var_percent[1:3]) #95.2
```

```{r, echo = F, warnings = F, fig.align='center', fig.width = 4, fig.height = 4}
#png(filename = paste0(file_path, "HSI_gsw_var_percent.png"), bg = "white", width = 10, height = 10, units = "in", res = 300)
...
#dev.off()
```
Figure 2: PCA scree plot. The first three PCs accounted for 95.2% of the variance. The top ten loadings in PC1, PC2 and PC3 are wavelengths in the regions of 708 to 721 nm, 1225 to 1275 nm and 2363 to 2497 nm.
...
HSI_top10_loadings <- data.frame(Trait = "gsw", PC = rep(c("PC1", "PC2", "PC3"), each = 10), 
...
#write.csv(file = paste0(file_path, "HSI_gsw_top_loadings.csv"), x= HSI_top10_loadings_2, row.names = F)
```
...
```
Figure 3: Frequency distributions of *gsw* ($molm^{-2}s^{-1}$) in the calibration (left, *n* = 74) and evaluation (right, *n* = 20) datasets. 
\newpage  

2. Fit a plsr model    
Model: gsw = Scores of PCs. The scores of the PCs were not scaled. 
The model was cross-validated with leave-one-out method.  
```{r, echo = F, fig.align='center', fig.width = 9, fig.height = 5}
plsr.ini <- plsr(gsw~inputs_scores, scale=FALSE,
                 validation="LOO",trace=FALSE,data=cal.plsr.data)
#png(filename = paste0(file_path, "gsw_nComps.png"), bg = "white", width = 10, height = 10, units = "in", res = 300)
par(mar = c(6, 7, 4, 2), mgp = c(5, 2, 0))
nComp <- selectNcomp(plsr.ini, method = "onesigma", plot = T, 
                     cex.axis = 2, cex.lab = 2, xlim = c(0, 15), ylim = c(0.10, 0.20), font.lab = 2, font = 2)
#dev.off()
```

---
title: "Use side-view HSI data to predict SLN"
date: "`r Sys.Date ()`"
output:
...
file_path <- "./results/reliefF_plsr/sideviewA/log_SLN_n_200_wv"
```
...
```{r data, echo = F}
y_metadata <- phyraw %>%
  select("ID", "SLN", "Treatment", "Subpop") %>%
  na.omit() %>%
  mutate(ID = as.factor(ID),
         log_SLN = log(SLN)) %>%
  select("ID", "log_SLN", "Treatment", "Subpop")
...
pre_cal = input_for_reliefF(X_IDdata = X_IDdata, y_metadata = y_metadata, 
                            cal.ID = cal.ID, y_name = 'log_SLN', 
                            num_X_names = num_X_names)

reliefF = reliefF_algorithm(pre_cal = pre_cal, 
                                y_name = 'log_SLN', num_X_names = num_X_names) 
...
```{r log SLN spectra in cal and val data sets, echo=FALSE, fig.align='center', fig.width = 9, fig.height = 6}
...
cal_ref_95perc = ggplot()+
  ...
  labs(title = "log(SLN) calibration dataset", x= "Wavelength (nm)", y = "Reflectance (%)") +
...
val_ref_95perc = ggplot()+
  ...
  scale_y_continuous(breaks = seq(sel_wv_ref_min, sel_wv_ref_max, length.out = 3), 
                     labels = round(seq(sel_wv_ref_min, sel_wv_ref_max, length.out = 3), 3),
                     limits = c(sel_wv_ref_min, sel_wv_ref_max)) +
  labs(title = "log(SLN) validation dataset", x= "Wavelength (nm)", y = "Reflectance (%)") +
  theme_bw(base_size=18) +
  theme(
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
...
```{r log SLN number of components CV, echo = F, fig.align='center', fig.width = 4, fig.height = 5}
plsr.ini <- plsr(log_SLN~Spectra, scale=FALSE,
                 validation="LOO",trace=FALSE,data=cal.plsr.data)
maxComps <- 15
#pdf(file = paste0(file_path, "/log_SLN_nComps.pdf"), bg = "white", width = 7, height = 5)
...
```{r log SLN plsr output, echo = F}
plsr.out <- plsr(log_SLN~Spectra, scale=FALSE, ncomp = nComps,
...
```{r log SLN jackknife coef, echo=FALSE}
jackknife = spectratrait::f.coef.valid(plsr.out, 
                                             cal.plsr.data, 
                                             nComps, 'log_SLN')
intercepts = jackknife[1,,,]
coefs = jackknife[-1,,,]
pred_jackknife = matrix(rep(intercepts, length(val.plsr.data[,'log_SLN'])), byrow = T,
...
```{r log SLN cal scatter, echo=FALSE}
...
```{r log SLN cal residuals plot, echo=FALSE}
...
```{r log SLN val CI plot, echo=FALSE}
val_CI_plot <- ggplot(val.plsr.output, aes(x=Observed, y=Predicted)) +
  geom_abline(intercept = 0, slope = 1, linewidth=0.3) + 
  geom_point(aes(col=Subpop, shape = Treatment)) + 
  scale_shape_manual(values = treatment_shape)+
  geom_errorbar(aes(ymin = lci, ymax = uci,col=Subpop), width=0.3, size = 0.1) +
  scale_color_manual(values = subpop_col)+
  #geom_smooth(method = "lm", formula = y ~ x, se = F) +
  scale_x_continuous(breaks = seq(round(rng[1]), round(rng[2]), length = 5), 
                     labels = seq(round(rng[1]), round(rng[2]), length = 5), 
...
```{r log SLN val PI plot, echo=FALSE}
val_PI_plot <- ggplot(val.plsr.output, aes(x=Observed, y=Predicted)) +
  geom_abline(intercept = 0, slope = 1, linewidth=0.3) + 
  geom_point(aes(col=Subpop, shape = Treatment)) + 
  scale_shape_manual(values = treatment_shape)+
  geom_errorbar(aes(ymin = lpi, ymax = upi, col=Subpop), width=0.3, size = 0.1) +
  scale_color_manual(values = subpop_col)+
  #geom_smooth(method = "lm", formula = y ~ x, se = F) +
  scale_x_continuous(breaks = seq(round(rng[1]), round(rng[2]), length = 5), 
                     labels = seq(round(rng[1]), round(rng[2]), length = 5), 
...
```{r log SLN val residuals plot, echo=FALSE}
...
```{r log SLN save plots and data, eval=FALSE, echo = F}
ggsave(filename = paste0(file_path, "/log_SLN_cal_scatter_plot.pdf"), 
       plot = cal_scatter_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/log_SLN_cal_resid_plot.pdf"), 
       plot = cal_resid_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/log_SLN_val_CI_plot.pdf"), 
       plot = val_CI_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/log_SLN_val_PI_plot.pdf"), 
       plot = val_PI_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/log_SLN_val_resid_plot.pdf"), 
       plot = val_resid_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/log_SLN_selected_wv.pdf"), 
       plot = selected_wv, 
       device = "pdf", width = 7, height = 5, units = "cm", dpi = 400)

write.csv(x=reliefF, 
          file = paste0(file_path, "/log_SLN_reliefF_200.csv"), 
          row.names = F)
write.csv(x=cal.plsr.output, 
          file = paste0(file_path, "/log_SLN.cal.plsr.output.csv"), 
          row.names = F)
write.csv(x=val.plsr.output, 
          file = paste0(file_path, "/log_SLN.val.plsr.output.csv"), 
          row.names = F)
write.csv(x=out.jk.coefs, 
          file = paste0(file_path, "/log_SLN_out.jk.coefs.csv"), 
          row.names = F)
write.csv(x=coefs, 
          file = paste0(file_path, "/log_SLN_coefs.csv"), 
          row.names = T)
```

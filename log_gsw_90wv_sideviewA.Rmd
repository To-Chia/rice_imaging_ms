---
title: "Use side-view HSI data to predict log gsw"
date: "`r Sys.Date ()`"
output:
...
file_path <- "./results/reliefF_plsr/sideviewA/log_gsw_n_90_wv"
```
...
```{r data, echo = F}
y_metadata <- phyraw %>%
  select("ID", "log_gsw", "Treatment", "Subpop") %>%
...
pre_cal = input_for_reliefF(X_IDdata = X_IDdata, y_metadata = y_metadata, 
                            cal.ID = cal.ID, y_name = 'log_gsw', 
                            num_X_names = num_X_names)

reliefF = reliefF_algorithm(pre_cal = pre_cal, 
                                y_name = 'log_gsw', num_X_names = num_X_names) 
reliefF_star =  reliefF[order(reliefF$value, decreasing = T)[1:90], ] 
tail(reliefF_star)
```
...
cal_ref_95perc = ggplot()+
  ...
  labs(title = "log(gsw) calibration dataset", x= "Wavelength (nm)", y = "Reflectance (%)") +
...
val_ref_95perc = ggplot()+
  ...
  scale_y_continuous(breaks = seq(sel_wv_ref_min, sel_wv_ref_max, length.out = 3), 
                     labels = round(seq(sel_wv_ref_min, sel_wv_ref_max, length.out = 3), 3),
                     limits = c(sel_wv_ref_min, sel_wv_ref_max)) +
  labs(title = "log(gsw) validation dataset", x= "Wavelength (nm)", y = "Reflectance (%)") +
  theme_bw(base_size=18) +
  theme(
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
...
plsr.ini <- plsr(log_gsw~Spectra, scale=FALSE,
                 validation="LOO",trace=FALSE,data=cal.plsr.data)
maxComps <- 15
pdf(file = paste0(file_path, "/log_gsw_nComps.png"), bg = "white", width = 7, height = 5)
...
plsr.out <- plsr(log_gsw~Spectra, scale=FALSE, ncomp = nComps,
...
coefs = jackknife[-1,,,]
pred_jackknife = matrix(rep(intercepts, length(val.plsr.data[,'log_gsw'])), byrow = T,
...
ggsave(filename = paste0(file_path, "/log_gsw_cal_scatter_plot.pdf"), 
       plot = cal_scatter_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/log_gsw_cal_resid_plot.pdf"), 
       plot = cal_resid_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/log_gsw_val_CI_plot.pdf"), 
       plot = val_CI_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/log_gsw_val_PI_plot.pdf"), 
       plot = val_PI_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/log_gsw_val_resid_plot.pdf"), 
       plot = val_resid_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/log_gsw_selected_wv.pdf"), 
       plot = selected_wv, 
       device = "pdf", width = 7, height = 5, units = "cm", dpi = 400)

write.csv(x=reliefF, 
          file = paste0(file_path, "/log_gsw_reliefF_90.csv"), 
          row.names = F)
write.csv(x=cal.plsr.output, 
          file = paste0(file_path, "/log_gsw.cal.plsr.output.csv"), 
          row.names = F)
write.csv(x=val.plsr.output, 
          file = paste0(file_path, "/log_gsw.val.plsr.output.csv"), 
          row.names = F)
write.csv(x=out.jk.coefs, 
          file = paste0(file_path, "/log_gsw_out.jk.coefs.csv"), 
          row.names = F)
write.csv(x=coefs, 
          file = paste0(file_path, "/log_gsw_coefs.csv"), 
          row.names = T)
```

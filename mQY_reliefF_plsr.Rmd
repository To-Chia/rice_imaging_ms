---
title: "Predict midday QY with hyperspectral data"
date: "`r Sys.Date ()`"
output: pdf_document
---
...
2. Built a PLSR model with $W_{PLSR}$ as input and midday QY (pQY, *n* = 94) as output. Methods were adapted from Burnett et al. (2021).  
...
inVar <- "mQY"
file_path <- "./results/reliefF_plsr/mQY/"
```
...
ref_trait_expRank <- attrEval(mQY ~., ref_trait_num, estimator = "RReliefFexpRank", ReliefIterations = nrow(trait)*50) 
...
mQY_feature_plot
```
Figure 1: Attributes values of HSI data on Week 10 for mQY (*n* = 94). Wavelengths with the top 300 attribute values are considered as important (red, $W_{PLSR}$) and are used for model input. 

```{r att anotate output, echo=FALSE, eval = F, fig.align='center', fig.width = 10, fig.height = 4}
mQY_feature_plot
ggsave(filename = paste0(file_path, "mQY_feature.png"), plot = mQY_feature_plot, device = "png", width = 30, height = 12, units = "cm", dpi = 400)
```
...
val_hist_plot <- qplot(val.plsr.data$mQY, geom="histogram", bins =5,
                       main = "Val. (n = 20)",
                       xlab = "midday QY", ylab = "Count", 
                       fill=I("#0072B2"),col=I("black")) + theme_bw(base_size = 18)
...
```
Figure 3: Frequency distributions of mQY in the calibration (left, *n* = 74) and validation (right, *n* = 20) datasets. 

2. Fitted a plsr model  
```{r determine number of components, echo = F, fig.align='center', fig.width = 9, fig.height = 7}
plsr.ini <- plsr(mQY~Spectra, scale=FALSE,
...
#png(filename = paste0(file_path, "mQY_nComps.png"), bg = "white", width = 7, height = 5, units = "in", res = 400)
...
cal_mQY_range <- (max(cal.plsr.output$mQY)-min(cal.plsr.output$mQY))
cal.NRMSEP <-  round((sqrt(mean(cal.plsr.output$PLSR_CV_Residuals^2))/cal_mQY_range)*100,3)

val.plsr.output <- data.frame(mQY = val.plsr.data$mQY,
...
val.plsr.output <- val.plsr.output %>%
  mutate(PLSR_Residuals = PLSR_Predicted-mQY)
...
val_mQY_range <- (max(val.plsr.output$mQY)-min(val.plsr.output$mQY))
val.NRMSEP <- round((sqrt(mean(val.plsr.output$PLSR_Residuals^2))/val_mQY_range)*100,3)
```
\newpage  
...
plot(cal.plsr.output.N1$PLSR_CV_Predicted, cal.plsr.output.N1[,"mQY"], 
       lwd=1.6, xlim=c(rng_vals[1], rng_vals[2]), ylim=c(rng_vals[1], rng_vals[2]), 
       pch=21, col="black", bg = tCol[1], cex=2, 
       xlab=paste0("CV Predicted ", inVar),
       ylab=paste0("Measured ", inVar),
       cex.axis=3, cex.lab = 3, font.lab = 2, font = 2)
points(cal.plsr.output.N2$PLSR_CV_Predicted, cal.plsr.output.N2[,"mQY"], 
...
box(lwd=2.2)
#dev.copy(png, file.path(paste0(file_path, "mQY_Cal_model.png")), height=2800, width=3200, res=340)
#dev.off()
```
Figure 4: Measured values  versus cross-validation values in calibration dataset (*n* = 74) of mQY.   
...
box(lwd=2.2)
#dev.copy(png, file.path(paste0(file_path, "mQY_cal_resi.png")), height=2800, width=3200, res=340)
#dev.off()

```
Figure 5: Residuals from the cross validation values versus cross validation predicted values (*n* = 70) of mQY.  
...
par(mfrow=c(1,1), mar=c(6, 7, 1, 0.4), oma=c(0, 0.1, 0, 0.2), mgp = c(5, 2, 0)) 
plotrix::plotCI(val.plsr.output.N1$PLSR_Predicted, val.plsr.output.N1[,"mQY"], 
       ...
       xlab=paste0("Predicted ", inVar),
       ylab=paste0("Measured ", inVar),
       cex.axis=3, cex.lab = 3, font.lab = 2, font = 2)
plotrix::plotCI(val.plsr.output.N2$PLSR_Predicted, val.plsr.output.N2[,"mQY"], add = T,
...
legend("bottomright", inset = c(0.1,0.1), legend = c("N1", "N2"), pch=21 , pt.bg = tCol,
       col = "black", cex=2, pt.cex = 2, text.col = "black", y.intersp = 1.5, bty = "n")
box(lwd=2.2)
#dev.copy(png, file.path(paste0(file_path, "mQY_val_model_with_95CI.png")), height=2800, width=3200, res=340)
#dev.off()
```
...
abline(h=0,lty=2,lw=2)
box(lwd=2.2)
#dev.copy(png, file.path(paste0(file_path, "mQY_val_resid_vs_pred.png")), height=2800, width=3200, res=340)
#dev.off()

```
Figure 7: Residuals from the external validation values versus predicted values (*n* = 20) of mQY.  

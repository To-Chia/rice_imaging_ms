---
title: "Use side-view HSI data to predict biomass"
date: "`r Sys.Date ()`"
output:
...
file_path <- "./results/reliefF_plsr/sideviewA/norm_FB_n_300_wv"
```
...
```{r data, echo = F}
y_metadata <- phyraw %>%
  select("ID", "FB", "Treatment", "Subpop") %>%
  na.omit() %>%
  mutate(ID = as.factor(ID),
         m_FB = mean(FB),
         sd_FB = sd(FB),
         norm_FB = (FB-m_FB)/sd_FB) %>%
  select("ID", "norm_FB", "Treatment", "Subpop") 
str(y_metadata)
# spectral data deviated away in ID 904, 929 and 990
y_metadata = y_metadata[-which(y_metadata$ID %in% c('904', '929', '990')), ]
...
pre_cal = input_for_reliefF(X_IDdata = X_IDdata, y_metadata = y_metadata, 
                            cal.ID = cal.ID, y_name = 'norm_FB', 
                            num_X_names = num_X_names)

reliefF = reliefF_algorithm(pre_cal = pre_cal, 
                                y_name = 'norm_FB', num_X_names = num_X_names) 
...
cal_val_list = cal_val_data_sets(cal.ID = cal.ID, val.ID = val.ID,
                                   ...
                                   X_attr = reliefF_star, 
                                   y_name = 'norm_FB', num_X_names = num_X_names,
                                   plot = T, y_unit = " (normalized final biomass)")
...
cal_ref_95perc = ggplot()+
  ...
  labs(title = "norm_FB calibration dataset", x= "Wavelength (nm)", y = "Reflectance (%)") +
...
val_ref_95perc = ggplot()+
  ...
  scale_y_continuous(breaks = seq(sel_wv_ref_min, sel_wv_ref_max, length.out = 3), 
                     labels = round(seq(sel_wv_ref_min, sel_wv_ref_max, length.out = 3), 3),
                     limits = c(sel_wv_ref_min, sel_wv_ref_max)) +
  labs(title = "norm_FB validation dataset", x= "Wavelength (nm)", y = "Reflectance (%)") +
  theme_bw(base_size=18) +
  theme(
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
...
plsr.ini <- plsr(norm_FB~Spectra, scale=FALSE,
                 validation="LOO",trace=FALSE,data=cal.plsr.data)
maxComps <- 15
#pdf(file = paste0(file_path, "/norm_FB_nComps.pdf"), bg = "white", width = 7, height = 5)
...
```{r norm_FB plsr output, echo = F}
plsr.out <- plsr(norm_FB~Spectra, scale=FALSE, ncomp = nComps,
...
coefs = jackknife[-1,,,]
pred_jackknife = matrix(rep(intercepts, length(val.plsr.data[,'norm_FB'])), byrow = T,
...
val_CI_plot <- ggplot(val.plsr.output, aes(x=Observed, y=Predicted)) +
  ...
  geom_point(aes(col=Subpop, shape = Treatment)) + 
  scale_shape_manual(values = treatment_shape)+
  geom_errorbar(aes(ymin = lci, ymax = uci,col=Subpop), width=0.5, size = 0.1) +
  scale_color_manual(values = subpop_col)+
  #geom_smooth(method = "lm", formula = y ~ x, se = F) +
  scale_x_continuous(breaks = seq(round(rng[1]), round(rng[2]), length = 5), 
                     labels = seq(round(rng[1]), round(rng[2]), length = 5), 
...
val_PI_plot <- ggplot(val.plsr.output, aes(x=Observed, y=Predicted)) +
  ...
  geom_point(aes(col=Subpop, shape = Treatment)) + 
  scale_shape_manual(values = treatment_shape)+
  geom_errorbar(aes(ymin = lpi, ymax = upi, col=Subpop), width=0.5, size = 0.1) +
  scale_color_manual(values = subpop_col)+
  #geom_smooth(method = "lm", formula = y ~ x, se = F) +
  scale_x_continuous(breaks = seq(round(rng[1]), round(rng[2]), length = 5), 
                     labels = seq(round(rng[1]), round(rng[2]), length = 5), 
...
ggsave(filename = paste0(file_path, "/norm_FB_cal_scatter_plot.pdf"), 
       plot = cal_scatter_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/norm_FB_cal_resid_plot.pdf"), 
       plot = cal_resid_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/norm_FB_val_CI_plot.pdf"), 
       plot = val_CI_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/norm_FB_val_PI_plot.pdf"), 
       plot = val_PI_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/norm_FB_val_resid_plot.pdf"), 
       plot = val_resid_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/norm_FB_selected_wv.pdf"), 
       plot = selected_wv, 
       device = "pdf", width = 7, height = 5, units = "cm", dpi = 400)

write.csv(x=reliefF, 
          file = paste0(file_path, "/norm_FB_reliefF_300.csv"), 
          row.names = F)
write.csv(x=cal.plsr.output, 
          file = paste0(file_path, "/norm_FB.cal.plsr.output.csv"), 
          row.names = F)
write.csv(x=val.plsr.output, 
          file = paste0(file_path, "/norm_FB.val.plsr.output.csv"), 
          row.names = F)
write.csv(x=out.jk.coefs, 
          file = paste0(file_path, "/norm_FB_out.jk.coefs.csv"), 
          row.names = F)
write.csv(x=coefs, 
          file = paste0(file_path, "/norm_FB_coefs.csv"), 
          row.names = T)
```

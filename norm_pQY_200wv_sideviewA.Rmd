---
title: "Use side-view HSI data to predict pQY"
date: "`r Sys.Date ()`"
output:
...
file_path <- "./results/reliefF_plsr/sideviewA/norm_pQY_n_200_wv"
```
...
```{r normalized pQY data, echo = F}
y_metadata <- phyraw %>%
  select("ID", "pQY", "Treatment", "Subpop") %>%
  na.omit() %>%
  mutate(ID = as.factor(ID),
         m_pQY = mean(pQY),
         sd_pQY = sd(pQY),
         norm_pQY= (pQY-m_pQY)/sd_pQY) %>%
  select("ID", "norm_pQY", "Treatment", "Subpop") 
...
pre_cal = input_for_reliefF(X_IDdata = X_IDdata, y_metadata = y_metadata, 
                            cal.ID = cal.ID, y_name = 'norm_pQY', 
                            num_X_names = num_X_names)

reliefF = reliefF_algorithm(pre_cal = pre_cal, 
                                y_name = 'norm_pQY', num_X_names = num_X_names) 
...
cal_ref_95perc = ggplot()+
  ...
  labs(title = "norm_pQY calibration dataset", x= "Wavelength (nm)", y = "Reflectance (%)") +
...
val_ref_95perc = ggplot()+
  ...
  scale_y_continuous(breaks = seq(sel_wv_ref_min, sel_wv_ref_max, length.out = 3), 
                     labels = round(seq(sel_wv_ref_min, sel_wv_ref_max, length.out = 3), 3),
                     limits = c(sel_wv_ref_min, sel_wv_ref_max)) +
  labs(title = "norm_pQY validation dataset", x= "Wavelength (nm)", y = "Reflectance (%)") +
  theme_bw(base_size=18) +
  theme(
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
...
plsr.ini <- plsr(norm_pQY~Spectra, scale=FALSE,
                 validation="LOO",trace=FALSE,data=cal.plsr.data)
maxComps <- 15
#pdf(file = paste0(file_path, "/norm_pQY_nComps.pdf"), bg = "white", width = 7, height = 5)
...
plsr.out <- plsr(norm_pQY~Spectra, scale=FALSE, ncomp = nComps,
...
coefs = jackknife[-1,,,]
pred_jackknife = matrix(rep(intercepts, length(val.plsr.data[,'norm_pQY'])), byrow = T,
...
ggsave(filename = paste0(file_path, "/norm_pQY_cal_scatter_plot.pdf"), 
       plot = cal_scatter_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/norm_pQY_cal_resid_plot.pdf"), 
       plot = cal_resid_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/norm_pQY_val_CI_plot.pdf"), 
       plot = val_CI_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/norm_pQY_val_PI_plot.pdf"), 
       plot = val_PI_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/norm_pQY_val_resid_plot.pdf"), 
       plot = val_resid_plot, 
       device = "pdf", width = 3.5, height = 3.5, units = "cm", dpi = 400)

ggsave(filename = paste0(file_path, "/norm_pQY_selected_wv.pdf"), 
       plot = selected_wv, 
       device = "pdf", width = 7, height = 5, units = "cm", dpi = 400)

write.csv(x=reliefF, 
          file = paste0(file_path, "/norm_pQY_reliefF_200.csv"), 
          row.names = F)
write.csv(x=cal.plsr.output, 
          file = paste0(file_path, "/norm_pQY_cal.plsr.output.csv"), 
          row.names = F)
write.csv(x=val.plsr.output, 
          file = paste0(file_path, "/norm_pQY_val.plsr.output.csv"), 
          row.names = F)
write.csv(x=out.jk.coefs, 
          file = paste0(file_path, "/norm_pQY_out.jk.coefs.csv"), 
          row.names = F)
write.csv(x=coefs, 
          file = paste0(file_path, "/norm_pQY_coefs.csv"), 
          row.names = T)
```
